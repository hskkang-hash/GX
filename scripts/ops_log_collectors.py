...
PROJECT_CONTAINERS = ("gx-shell", "postgres", "redis",
                      ...
                      # ★ [실측 2026-09-24 · 턴 AG · 조율자] ⑤가 **또** 같은 얼굴로
                      #   빨갰다. 조율자가 09-23 에 `gx-portfwd` 를 세워 놓고 이
                      #   목록에 안 넣었다 — 대표 브라우저가 컨테이너 안 게이트
                      #   서버(SPA 3002 · runserver 8000)에 닿게 하는 중계기다.
                      #   ⚠ **「곧 지울 것」이라서 빼지 않는다.** 이 목록이 묻는 것은
                      #     「오래 살 것인가」가 아니라 **「지금 stdout 에 쌓고 있는가」**다.
                      #     임시라는 말로 빼기 시작하면 임시가 제일 오래 남는다.
                      #   ★ 이 줄과 함께 그 컨테이너를 `--log-opt` 를 붙여 다시 띄웠다
                      #     (붙이기 전 `opts=map[]` — 상한 없이 무한정 쌓고 있었다).
                      "gx-portfwd")
