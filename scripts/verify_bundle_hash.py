...
def judge(bundle: dict | None, commit: str | None,
    ...
    if not commit:
        out.append((n_eq, GRAY,
                    "**못 쟀다** — 현재 커밋을 모른다. `.git/HEAD` 를 못 읽었고 "
                    "`GX_COMMIT` 도 없다 (컨테이너 안에서는 환경변수로 넣는다)"))
    elif len(found) != 1:
        out.append((n_eq, GRAY,
        ...
        out.append((n_eq, PASS,
                    f"{found[0][:SHORT]} = {commit[:SHORT]} — 서버가 내는 번들이 "
                    f"**현재 커밋**이다"))
    else:
        out.append((n_eq, FAIL,
                    f"번들 {found[0][:SHORT]} ≠ 현재 {commit[:SHORT]} — **서버가 내는 "
...
def self_test() -> int:
    ...
    if bad:
        ...
        for b in bad:
            print("    " + b)
        return EXIT_FAIL
    print(f"{TAG} 자기시험 통과 — **출생 표본 3**(턴 D · 낡은 사본 · 말없는 번들) · "
          f"양성 2 · 음성 2 · 회색 2 · 곁파일 2 · 갈래 2 · 토큰 2 · 파싱 1")
    return EXIT_OK
...
def main() -> int:
    ...
    rows = judge(bundle, commit, sidecar)
