...
★ 이 스크립트가 답해야 했던 질문은 2026-09-06 **신청서 유형으로 먼저 답해졌다** (D-329)
----------------------------------------------------------------------------------------
발급된 승인키 2건의 API 유형이 둘 다 **「도로명주소 팝업 API」**였다
[실측·대표 승인 화면 2건]. 팝업 API 는 브라우저에 주소검색 창을 띄우는 UI 위젯이고
서버끼리 주고받는 조회 API 조차 아니다 — 그래서 **호출하기 전에 답이 났다.**
`JUSO_REVERSE_SUPPORTED='no'` 로 확정됐고, FX-5 는 카메라 설치 주소로 대체됐다(D-330).

**그래도 이 파일을 지우지 않는다.** 유형이 다른 키(주소검색 API·좌표변환 API)가 생기면
그때 이 스크립트가 그대로 쓰인다. 지운 것을 지운 채로 두면 다음 사람이 다시 만든다.

★ 그리고 남는 교훈 하나: 우리는 **「키가 있는가」를 물었고 「어떤 키인가」는 묻지 않았다.**
  그 교정이 표 ②의 `api_type`·`capability` 칸과 상태 5값이다(D-328).
