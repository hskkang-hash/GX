...
if __name__ == "__main__":
    ...
    #: 다섯 조건(①~⑤) 자체는 지금도 매번 세어진다(`conditions()`) — 회색인 이유는
    #: 「합격선이 무엇인가」(SDN 제외판 릴리스 후보를 선언해도 되는 **언제**)가 아직
    #: 결정문에 없어서다. 세종이 그 「언제」를 정했다: **RC-1 컷 2026-09-25**
    #: (§4 세종 산출물 · RC-1 선언문 초안). 그날 이 게이트를 다시 배선한다 — 그때까지는
    #: 여전히 회색이고, D-511 기한(09-23)이 와도 다른 회색과 함께 빨강이 된다(면제가 아니다).
    gate_header(
        __file__,
        measured=("deferred:RC-1 2026-09-25 · 무엇을: 릴리스 후보(SDN 제외판) 도달 조건 "
                  "다섯(①~⑤)의 **합격선**. 조건 자체는 `conditions()` 가 지금도 매번 "
                  "센다 — 못 정한 것은 「그 다섯이 찼으면 후보를 선언해도 되는가」의 "
                  "기준 시점이다. RC-1 선언문 초안(세종 · 09-25)이 그 기준을 낸다"),
    )
    sys.exit(main())
