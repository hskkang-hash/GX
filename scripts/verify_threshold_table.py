...
DECLARED_NOT_A_SETTING: dict[str, str] = {
    # ── 2026-09-24 · 2파 병합에서 태어난 열다섯 ────────────────────────────
    #   ★ 판정 기준은 머리말 그대로다: **계약도 운영도 부르지 않는 값**만 여기 적는다.
    #     아래는 셋으로 갈린다 — 사유 칸이 어느 갈래인지 말한다:
    #       ㉠ 부르는 쪽이 정하는 기본값(페이지 크기·창 인자)
    #       ㉡ 저장 형식·안전 한도 (운영이 조절할 값이 아니다)
    #       ㉢ **표 ① 이관 대상** — 규칙의 수인데 아직 함수 인자로 받는다. 사유에 그대로 적는다
    "apps/dsm/api.py::기본값::days::30#1":
        "㉠ 월간 대응 시간 창의 기본값 — 화면이 `?days=` 로 정한다(UX-14).",
    "apps/dsm/api.py::기본값::limit::200#1":
        "㉠ 초점 큐 페이지 크기의 기본값. 부르는 쪽이 `?limit=` 로 정한다.",
    "apps/dsm/api.py::비교::days::366#1":
        "㉡ 대응 시간 조회의 **상한 방어**(1년). 운영이 조절하는 문턱이 아니라 "
        "「한 번에 얼마까지 물어도 되는가」의 안전 한도다.",
    "apps/dsm/api.py::비교::limit::1000#1":
        "㉡ 초점 큐 한 번 조회의 상한 방어. 위와 같은 갈래다.",
    "apps/dsm/services.py::기본값::limit::200#1":
        "㉠ 위 라우트가 넘기는 같은 값 — 두 자리가 같은 뜻이므로 함께 선언한다.",
    "apps/dsm/services.py::이름대입::GROUP_WINDOW_SECONDS::300#1":
        "㉢ **표 ① 이관 대상.** UX-13 ×N 묶음의 창(5분)이다. PRD v2.5 가 「5분 창」으로 "
        "이름을 댄 규칙의 수이므로 운영이 조절할 값이 맞다. 지금은 `focus_queue` 가 "
        "인자로 받고 이 상수는 그 기본값이다 — 표 ①로 옮기는 커밋에서 이 줄을 지운다.",
    "kernels/k2_notify/alarm_budget.py::이름대입::BUDGET_WINDOW_DAYS::7#1":
        "㉢ **표 ① 이관 대상.** QA-12 알림 예산이 보는 창(최근 7일). PRD 가 이름을 댔다. "
        "시간당 상한 6건은 EEMUA 191 [인용]이라 우리 표에 올릴 수가 아니지만, "
        "**창의 길이는 우리가 정한다.**",
    "kernels/k2_notify/heartbeat.py::시간인자::timedelta(hours=)::2#1":
        "㉢ **표 ① 이관 대상.** OPS-14 생존 알림의 **유예**(08:00 + 2시간). 이 값이 "
        "「아직 안 왔다」와 「늦었다」를 가른다 — 짧으면 매일 아침 거짓 경보가 뜨고 "
        "그러면 이 신호가 꺼진다(D-290). 운영이 조절할 값이 맞다.",
    "stream_monitors/management/commands/seed_role_users.py::시간인자::urlopen(timeout=)::60#1":
        "㉡ 시드가 사용자 생성 API 를 부를 때의 타임아웃. 운영 경로가 아니라 "
        "**검수용 시드 커맨드**의 값이다.",
    "stream_monitors/services/bulk_register.py::이름대입::MAX_ROWS::500#1":
        "㉡ CSV 한 번에 받는 행의 **안전 한도**(UX-18). 운영이 조절하는 문턱이 아니라 "
        "한 요청이 만들 수 있는 일의 상한이다 — 늘리려면 dry-run 표의 크기와 "
        "트랜잭션 경계를 함께 봐야 한다.",
    "stream_monitors/services/camera_pulse.py::시간인자::timedelta(minutes=)::5#1":
        "㉢ **표 ① 이관 대상.** OPS-15 `PULSE_TIMEOUT` — 마지막 프레임 뒤 이만큼 지나면 "
        "맥박을 잃었다고 본다. PRD 가 「5분」으로 이름을 댄 규칙의 수다. "
        "지금은 `scan_clusters(timeout=)` 이 인자로 받고 이 상수는 기본값이다.",
    "stream_monitors/services/camera_pulse.py::시간인자::timedelta(minutes=)::5#2":
        "㉢ **표 ① 이관 대상.** `CLUSTER_WINDOW` — 「동시에」의 폭. 위와 같은 갈래이고 "
        "**같은 5분이지만 다른 뜻**이라 상수가 둘이다(하나로 묶으면 한쪽만 바꾸고 싶은 "
        "날 둘 다 바뀐다).",
    "stream_monitors/services/camera_pulse.py::시간인자::timedelta(minutes=)::5#3":
        "㉢ **표 ① 이관 대상.** `CLUSTER_RENOTIFY_WINDOW` — 같은 군집을 이 시간 안에 "
        "다시 내지 않는다. 1분마다 도는 검사가 5분 동안 5건을 만들면 **하나의 사건이 "
        "다섯 줄**이 된다.",
    "stream_monitors/services/camera_pulse.py::이름대입::CLUSTER_MIN_CAMERAS::3#1":
        "㉢ **표 ① 이관 대상.** OPS-15 의 N — 이보다 작은 구역은 군집 판정을 하지 않는다. "
        "PRD 가 「기본 3」이라고 적었다: **기본값이라는 말 자체가 운영이 바꾼다는 뜻**이다.",
    "stream_monitors/services/camera_pulse.py::이름대입::CLUSTER_MIN_DOWN::2#1":
