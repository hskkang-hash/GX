...
if __name__ == "__main__":
    ...
    _n_py = sum(1 for _ in (ROOT / "backend").rglob("*.py"))
    ...
    gate_header(__file__, measured=(
        "외부 의존 호출(requests·httpx·minio·redis·celery 등)에 **타임아웃이 있는지** "
        "훑는다 — **분모 %d**(backend/ 의 .py 전수 · 지금 셌다). 걸린 호출 자리 수는 "
        "실행 중에 세어 아래 [TIMEOUT] 줄에 적는다" % _n_py))
