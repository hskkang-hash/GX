...
def judge_clause(cid: str, clause: dict, *, screens: list, gate_rc: dict) -> tuple[str, str]:
    """절 하나 → **(색, 사유)**. 이 함수만이 색을 만든다."""
    if (clause.get("state") or "").strip() == "잠김":
        return LOCKED, "계약이 조건부로 잠근 절 (%s) — 판정 대상이 아니다" % (
            clause.get("unlock_id") or "?")

    link = REACH_MAP.get(cid)
    if not link:
        why = GREY_WHY.get(cid)
        if not why:
...
def judge(observations: dict) -> list:
    ...
    if not clauses:
        return []
    return [(cid, *judge_clause(cid, c, screens=screens, gate_rc=gate_rc))
...
def observe(run_gates: bool = True) -> dict:
    ...
    wanted = {v["gate"] for v in REACH_MAP.values() if v.get("gate")}
    return {"clauses": clauses, "screens": screens,
...
def main() -> int:
    ...
    t = tally(rows)
    a = area_one(rows)
    print("%s [입력] 계약 절 %d개 · 화면 %d장 · 맡긴 게이트 %d벌"
