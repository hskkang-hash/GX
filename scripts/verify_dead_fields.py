...
DECLARED_UNWIRED: dict[str, str] = {
    ...
    **{f"stream_monitors.DsmReportRun.{f}": (
    ...
    # ── D-492 · 2026-09-18 턴 U 병합 — **사람이 실행하며 적는 칸 하나**
    "stream_monitors.DsmSystemRequest.handled_note":
        "D-492. 재시작 **요청**을 점검 창에서 실행한 사람이 적는 칸이다 — 요청하는 문"
        "(`POST /api/dsm/system/restart-request`)은 `status=requested` 로 행을 만들 뿐이고, "
        "**실행하는 문은 이 저장소에 없다**(운영계 행위는 대표의 몫 · WO-01 불변). "
        "쓰기 경로가 없는 것이 사고가 아니라 설계다: 이 앱이 이 칸을 채울 수 있으면 "
        "그것은 곧 이 앱이 재시작을 실행한다는 뜻이고, 그러면 「눌렀다」와 「일어났다」가 "
        "한 칸이 된다 — 그 표가 태어난 이유가 정확히 그 둘을 가르는 것이었다. "
        "★ 이 등재는 만료일이 적힌 이름표다: 점검 창 기록 문(운영자가 실행 결과를 "
        "적는 자리)이 서는 커밋에서 이 줄을 지우고 그 쓰기와 같은 커밋에 넣는다. "
        "그때 이 판정기가 먼저 「등재를 지워라」로 빨개진다.",
