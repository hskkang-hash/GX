...
API_PATHS = (
    ...
    #: ★ 2026-09-21 (P-178 U56 ①) — **감사 화면이 넓어졌다**: 이 라우트가 여는 채널이
    #:   둘에서 **열다섯**으로 늘었다(`apps/dsm/audit.READABLE_LOGGER_NAMES`). 그 쪽의
    #:   `reason` 칸은 감사 행의 `note` 를 **그대로** 내보내고, 그 글은 우리가 쓴
    #:   글이다 — 이 판정기가 안 보는 동안 「우리 언어」가 새기 가장 쉬운 자리다.
    #:   [실측 2026-09-21 · 2,066행 전수 · 패턴 6종 · **0건**] 그래서 넣는다:
    #:   넓히는 결정과 그 결정을 지키는 게이트가 **같은 턴에** 서야 한다.
    #:   ⚠ 읽을 권한이 없는 자격(U1·U3)으로 돌면 403 본문을 재고, 그 본문에도
    #:     패턴은 없다 — 판정이 자격에 따라 갈리지 않는다.
    "/api/dsm/audit",
)
