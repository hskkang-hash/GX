...
class RateWindow:
    ...
    since: datetime
    until: datetime
    #: 분자 — 사람이 **기각**한 이벤트 수. `DetectionEvent.verdict` 에서 센다.
    rejected: int
    #: 분모 — 사람이 **판정한** 이벤트 수 (확인 + 기각). 미판정은 분모가 아니다.
    #:
    #: ★ `status` 가 아니라 `verdict` 에서 센다 (D-293). status 는 종료가 덮으므로
    #:   그것으로 세면 **종료가 쌓일수록 분모가 줄고 오탐률이 저절로 좋아진다.**
    #:   개선된 것이 아니라 나쁜 데이터가 사라진 것이다.
    reviewed: int
    #: 아직 판정되지 않은 이벤트 수. 분모 밖이지만 **보이게 둔다** —
    #: 이 수가 크면 위의 비율은 표본이 얇다는 뜻이다.
    unreviewed: int = 0
    #: ★ 판정 없이 종료된 이벤트 수. **지워진 것이 아니라 처음부터 없던 것**이다 (D-293).
    #:
    #:   이름이 바뀐 내력을 남긴다: 예전 이름은 `verdict_lost_to_close` 였고, 종료가
    #:   판정을 **덮어 지우던** 시절의 손실을 세던 칸이다. D-293 이 그 덮어쓰기를
    #:   막았으므로 이제 그런 손실은 없고, 남은 것은 "아무도 판정하지 않은 채 닫힌 것"뿐이다.
    #:   그것은 손실이 아니라 미판정이므로 이름을 사실에 맞췄다.
    #:   여전히 보고한다 — 이 수가 크면 판정 절차가 종료에 밀리고 있다는 뜻이다.
    closed_without_verdict: int = 0
