...
class EventView:
    ...
    #: ★ 대응 진행 축 (D-399). **판정 축(`status`/`verdict`)과 다른 것을 묻는다** —
    #:   「사람이 어디까지 했나」다. 2026-09-21 에 목록으로 나오기 시작했다:
    #:   그전까지 이 값은 **상세에만** 있었고, 그래서 U2 의 「미처리 이벤트 확인」을
    #:   서버가 걸러 줄 수 없었다(온보딩 48행 U2 #2 · 착시 ⑨의 이웃 — 값은 있는데
    #:   나가는 문에 안 실린 것). 화면이 목록을 받아 자기가 세면 그 순간
    #:   **페이지에 없는 이벤트는 안 세어진다.**
    response_state: str = "occurred"
