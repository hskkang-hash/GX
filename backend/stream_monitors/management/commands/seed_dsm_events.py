...
# ═══════════════════════════════════════════════════════════════════════════
# P-223 — **시드 규약 한 줄: 시드는 종결(closed)된 채로 심는다**
# (2026-09-21 · 턴 AA · 차선 U1 · 세종 실사용 점검)
# ═══════════════════════════════════════════════════════════════════════════
#
# 세종이 고객 자리에 앉아 적었다:
#
#     「지금 처리할 것(큐)」 — 가장 급한 하나가 **15일 전 사건**이고 대응 시계가
#     **「377시간 27분」 빨강**이다. 제품은 옳다(오래 열린 것이 급하다). 그러나
#     고객 첫날 화면에 377시간은 **「이 시스템은 방치돼 있다」**로 읽힌다.
#
# ★ **시드는 늙는다.** 이것이 규약이 필요한 이유 전부다. 심을 때 「발생 · 미판정」인
#   행은 **아무도 안 누르면 영원히 열려 있고**, 시계는 계속 돈다. 검수하려고 심은
#   것이 검수가 끝난 뒤에는 **가장 급한 일감**이 되어 큐 맨 위에 선다 —
#   심은 날에는 「3분 경과」라 아무도 못 봤고, 열흘 뒤에 첫 화면을 먹었다.
#
# ★ 그래서 **열린 채로 심지 않는다.** 대응 축은 네 조합 모두 `closed` 까지 걷고,
#   **갈리는 것은 판정 축**(`verdict`)이다 — 두 축이 있다는 사실은 그대로 보인다.
#   화면이 보여 줘야 하는 것은 「이 제품에 미처리 사건이 있다」가 아니라
#   「이 제품은 사건을 **끝까지** 처리한다」이고, 그것은 닫힌 행이 말한다.
#
# ★ **미처리 한 건이 필요한 자리는 따로 심는다.** 온보딩 `U1#11`(「미처리 사건이
#   화면에 뜬다 · 접수 누름」)은 열린 사건이 있어야 잰다 — 그것은 **재는 순간에**
#   `data_source=drill` 로 심고 재고 닫는 일이고, 검수용 시드가 늙어서 대신할 일이
#   아니다. 잴 것을 미리 심어 두면 그 씨앗은 **안 잴 때도 화면에 서 있다.**
#
#: 대응 축 × 판정 축의 조합. **한 축만 채우면 두 축이 있다는 사실이 화면에 안 보인다.**
#: (대응 진행 몇 칸을 갈 것인가, 판정 verdict 를 붙일 것인가)
#: ⚠ 첫 칸의 `3` 은 **전부 종결**이라는 위 규약이다 — 줄이려면 규약을 먼저 고친다.
COMBOS = [
    (3, None),              # 종결 · 미판정 — 대응은 끝났고 진위는 안 적었다
    (3, None),              # 종결 · 미판정 (두 벌 — 목록에 한 줄만 있으면 축이 안 보인다)
    (3, "confirmed"),       # 종결 · 진짜였다
    (3, "confirmed"),       # 종결 · 진짜였다
