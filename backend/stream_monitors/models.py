...
class StreamMonitor(BaseModelWithGroup):
    ...
    # ── P-224 청구 출처 (2026-09-21 · 턴 AB · 차선 B) ─────────────────────
    #
    # 왜 칸 하나가 필요한가 — **우리가 심은 카메라에 고객이 돈을 내고 있었다**
    # ---------------------------------------------------------------------
    # [실측 2026-09-21 11:37] `ETRI-Group` 의 살아 있는 카메라 15대 중 **11대**가
    # 우리가 심은 것(게이트 탐침 7 · 검수 씨앗 1 · 훈련 3)이었고, 그 11대가 전부
    # 청구의 셈에 들어 있었다. 사건·발송은 `track_id` 에 얹힌 표식으로 이미 빠지는데
    # (P-193 · P-201) **카메라 표에는 표식을 걸 자리가 없었다.**
    #
    # ★ **기본값이 `live` 인 것이 규약이다** — 표식 없음 = **고객의 것**이다.
    #   반대로 두면 표식을 안 단 고객 카메라가 조용히 공짜가 되고, **공짜가 된 것은
    #   안 보인다**(틀린 청구는 고객이 항의하지만, 안 한 청구는 아무도 말 안 한다).
    #
    # ★ **이 칸은 제품을 감추지 않는다** (D-497). 관제 화면·큐·감사는 이 값을 안
    #   본다 — 바뀌는 것은 **청구의 셈**뿐이다. 감추면 그것은 「세지 않기」가 아니라
    #   카메라를 숨긴 것이고, 숨긴 카메라는 아무도 못 고친다.
    #
    # ⚠ **판단은 여기 없다.** 어느 값이 청구에서 빠지는가는 `common/billing_marks.py`
    #   한 곳이 안다(`NONBILLABLE_SOURCES`). 여기에 choices 를 못박으면 낱말이 두
    #   벌이 되고, 한쪽을 고칠 때 다른 쪽은 안 고쳐진다.
    data_source = models.CharField(
        max_length=32, default="live", db_index=True,
        help_text="청구 출처. live = 고객의 것(기본값) · probe/seed/drill = 우리가 "
                  "심은 것으로 청구에서 뺀다. 빼는 판단은 common/billing_marks.py "
                  "한 곳이 한다 (P-224)",
    )
