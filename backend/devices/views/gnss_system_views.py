from ninja_extra import api_controller, route
from typing import List
from ninja.errors import ValidationError
from common.pagination import OptimizedPaginator

from devices.models import GNSSSystem
from devices.schemas.schemas_djantic_in import GNSSSystemInSchema
from devices.schemas.schemas_djantic_out import GNSSSystemOutSchema
from core.api.v1.auth import CustomJWTAuth
from core.common.base_response import BaseResponse
from core.common.search.dynamic_search import apply_dynamic_filters
from common.constant import MESSAGE_ENUM
from common.inbound_api_key import JwtOrInboundKey

@api_controller('/gnss-systems', tags=['GNSS Systems'])
class GNSSSystemAPI:
    @route.get('', auth=JwtOrInboundKey())
    def list_gnss_systems(self, request):
        page_size = int(request.GET.get('page_size', 25))
        current_page = int(request.GET.get('current_page', 1))

        gnss_systems = GNSSSystem.objects.all().order_by('-id')
        gnss_systems = apply_dynamic_filters(gnss_systems, request.GET, [], request.GET.get('sort_obj', None))

        # 🚀 OPTIMIZED: Use OptimizedPaginator to automatically optimize COUNT query
        paginator = OptimizedPaginator(gnss_systems, page_size)
        pages = paginator.page(current_page)
        data = GNSSSystemOutSchema.from_queryset(pages.object_list, many=True, auto_resolve_fields=False)

        return BaseResponse(
            status_code=200,
            message=MESSAGE_ENUM.get(MESSAGE_ENUM.GET_LIST_GNSS_SYSTEM_SUCCESS),
            data=data,
            total_pages=paginator.num_pages,
            total_items=paginator.count,
            current_page=current_page
        )

    @route.get('/{id}', auth=JwtOrInboundKey())
    def get_gnss_system(self, id: int):
        try:
            gnss_system = GNSSSystem.objects.get(id=id)
            return BaseResponse(
                status_code=200,
                message=MESSAGE_ENUM.get(MESSAGE_ENUM.GET_GNSS_SYSTEM_DETAIL_SUCCESS),
                data=GNSSSystemOutSchema.from_queryset(gnss_system)
            )
        except GNSSSystem.DoesNotExist:
            return BaseResponse(
                status_code=404,
                message=MESSAGE_ENUM.get(MESSAGE_ENUM.NOT_FOUND, "GNSS system"),
                data=None
            )

    @route.post('', auth=CustomJWTAuth())
    def create_gnss_system(self, data: GNSSSystemInSchema):
        try:
            gnss_system = GNSSSystem.objects.create(**data.dict())
            return BaseResponse(
                status_code=200,
                message=MESSAGE_ENUM.get(MESSAGE_ENUM.CREATE_GNSS_SYSTEM_SUCCESS),
                data=GNSSSystemOutSchema.from_queryset(gnss_system)
            )
        except ValidationError as e:
            return {"success": False, "errors": e.errors()}

    @route.put('/{id}', auth=CustomJWTAuth())
    def update_gnss_system(self, id: int, data: GNSSSystemInSchema):
        try:
            gnss_system = GNSSSystem.objects.get(id=id)
            for key, value in data.dict().items():
                setattr(gnss_system, key, value)
            gnss_system.save()
            return BaseResponse(
                status_code=200,
                message=MESSAGE_ENUM.get(MESSAGE_ENUM.UPDATE_GNSS_SYSTEM_SUCCESS),
                data=GNSSSystemOutSchema.from_queryset(gnss_system)
            )
        except GNSSSystem.DoesNotExist:
            return BaseResponse(
                status_code=404,
                message=MESSAGE_ENUM.get(MESSAGE_ENUM.NOT_FOUND, "GNSS system"),
                data=None
            )
        except ValidationError as e:
            return {"success": False, "errors": e.errors()}

    @route.delete('/{id}', auth=CustomJWTAuth())
    def delete_gnss_system(self, id: int):
        try:
            gnss_system = GNSSSystem.objects.get(id=id)
            gnss_system.delete()
            return BaseResponse(
                status_code=200,
                message=MESSAGE_ENUM.get(MESSAGE_ENUM.ACTION_DELETE_SUCCESS),
                data=None
            )
        except GNSSSystem.DoesNotExist:
            return BaseResponse(
                status_code=404,
                message=MESSAGE_ENUM.get(MESSAGE_ENUM.NOT_FOUND, "GNSS system"),
                data=None
            )
