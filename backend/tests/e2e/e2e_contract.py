...
KERNEL_PACKAGES: dict[str, str] = {
    ...
    # ★ VIDEO 도 커널이 아니라 L3 Platform 의 서비스다. 2026-09-02 · D-306 으로 **열렸다.**
    #   잠금 사유였던 "이벤트↔영상 배선이 없다" 가 사라졌다 — EventClip(마이그 0021)이
    #   실재하고, 쓰기는 이벤트 생성 경로 안 한 곳에서 난다.
    #
    #   ⚠ **열린 것은 참조까지다.** 실제 구간 추출·전송은
    #   `stream_monitors.services.clips.CLIP_EXTRACTION_READY = False` 가 잠근다.
    #   두 잠금이 다른 것을 잠근다: 여기 것은 "단계를 잴 수 있는가",
    #   저기 것은 "바이트를 줄 수 있는가". 참조가 열렸다고 재생까지 열리지 않는다.
    "VIDEO": "stream_monitors.services.clips",
}
...
LOCKED_CAPABILITIES: dict[str, str] = {
    # ★ VIDEO 는 **여기서 지워졌다** (D-306, 2026-09-02).
    #   잠금 사유는 D-302 가 실측한 "배선 부재" 였다 — clip_path 쓰기 0곳 · 이벤트↔녹화
    #   연결 없음 · 재생 라우트 0건. 셋 다 해소됐다: EventClip 이 이벤트 생성 경로에서
    #   써지고(쓰기 1곳), 재생 라우트가 규약 넷을 시험으로 달고 섰다.
    #   지운 것이 곧 "이제 잴 수 있다"는 선언이고, 근거는 마이그레이션 0021 과
    #   backend/tests/test_clip_playback.py 다.
    #   되돌아온다면(배선이 지워진다면) 그 시험이 먼저 멈춘다.
    #
    #   ⚠ 재생 자체(바이트)는 여전히 잠겨 있다. 그것은 이 등재부가 아니라
    #   CLIP_EXTRACTION_READY 가 잠근다 — 잠기는 대상이 다르기 때문이다.
...
SCENARIOS: dict[str, Scenario] = {
    "E2E-1": Scenario(
        ...
        steps=(
            ...
            # ★ 제목이 바뀌었다 — **재는 것을 그대로 적는다.**
            #   "3초 이내 재생" 은 바이트가 나가야 잴 수 있고, 그것은 아직 잠겨 있다.
            #   지금 재는 것은 "클릭 → 어디를 보라를 안전하게 말하기" 까지다.
            #   원래 제목을 그대로 두고 초록을 내면 재지 않은 것을 잰 것으로 읽힌다(D-302).
            Step(9, "이벤트 클릭 → 영상 구간 참조 + 재생 규약 4종", "VIDEO",
                 ac="F-09 영상 참조(추출은 CLIP_EXTRACTION_READY 로 잠김)"),
