...
실측 모수는 **149종**이다 (저장소 관할 · `groups` M2M ∪ `group` FK).
...
**행이 있는 표만** 센다. 그래서 36종이 목록 밖에 있었다 (행 0 이 32종).
...
CENSUS: dict[str, tuple[str, str, str, int | None, str]] = {
    ...
    "stream_monitors.CredentialRecord":
        ("GAP", "no_route", "fk", 0,
         "인구조사(덤프 집계)에 없었다 — 행이 0이라 안 보였다. **아직 안 쓴 것이지 안전한 것이 아니다** / 도달: 실경로 추적본에 항목이 없고, 전수 라우트에도 이 모델을 가리키는 경로가 없다"),
    ...
    "stream_monitors.GradeRule":
        ("GAP", "via_parent", "fk", 0,
         "2026-09-10 D-368 로 신설(마이그 0023). 인구조사(2026-08-14 덤프)에 없었던 것이 아니라 그때 존재하지 않았다. 소유는 **바꾼 사람의 소속에서 물려받는다**(kernels.k5_trust.grade_rules._inherit_group) — 주인 없는 행은 §0.4 의 created_by__isnull OR 절을 타고 모두에게 보인다(W0-13 이 되돌린 상태). ★ 이 표의 격리가 특별히 중요한 이유: 행 하나가 **다른 테넌트의 경보를 끌 수 있다**(fire→info). 격리 단언은 backend/tests/test_grade_rules.py 의 GradeRulesAreTenantScopedTest 4건이 잰다 — 남의 규칙이 내 판정을 안 바꾸고, 내 목록에 안 보이고, 파이프라인이 테넌트를 추측하지 않고, 무권한이 못 바꾼다. 쓰기 면은 커널 함수 하나(set_grade_rule)이고 HTTP 는 POST /api/dsm/settings/grade-rules 한 곳 — 전건 @tenant_scoped + guard_setting / 도달: 자기 pk 로 지목하는 경로가 없다(event_type 으로만 지목한다). 전수 라우트에도 이 모델의 pk 를 받는 경로가 없다. D-272 재검사 [2026-09-07 턴 J · 차선 S] — 라우트가 실재하는 것이 맞다. reach 를 고친 근거는 gen_tenant_census.reach_of 의 규칙 그대로다(①자기 pk 경로 → direct_pk · ②pk 는 없으나 이름이 이 모델을 가리키는 경로 → via_parent). 옛 값 no_route 는 **틀린 낱말이 아니라 낡은 사실**이었다 — 그 줄을 쓸 때는 라우트가 없었고, 판정기가 읽던 사진이 2026-08-22(531라우트)라 705라우트에서 생긴 이 경로들을 두 달 동안 못 봤다. 닿는 경로: POST /api/dsm/settings/grade-rules (본문의 event_type 으로 지목 · pk 아님) → via_parent. 재검사 결과 [실측]: HTTP 층에서 tenant 를 요구한다 — GET 은 405(POST 만 존재)이고, 라우트는 auth=JwtOrInboundKey + @tenant_scoped 이며 핸들러가 scope=_scope(request) 를 services.set_grade_rule_value 에 그대로 넘긴다. 커널 층 격리 단언 9건 통과 (backend/tests/test_grade_rules.py::GradeRulesAreTenantScopedTest — test_a_human_without_a_group_is_refused_not_answered_globally 포함). ★ 못 쟀다(회색): 두 테넌트의 행을 놓고 HTTP 로 대조하는 시험 — 이 DB 는 이 표 행이 0이고 모든 계정의 group 이 None 이라 대조 자체가 성립하지 않는다. 0건을 「없다」로 읽지 않는다(D-301)"),
    ...
    "stream_monitors.ThresholdChange":
        ("GAP", "no_route", "fk", 0,
         "인구조사(덤프 집계)에 없었다 — 행이 0이라 안 보였다. **아직 안 쓴 것이지 안전한 것이 아니다** / 도달: 실경로 추적본에 항목이 없고, 전수 라우트에도 이 모델을 가리키는 경로가 없다"),
    ...
    "stream_monitors.WebhookSubscription":
        ("GAP", "direct_pk", "fk", 0,
         "2026-09-05 TC UX-19 로 신설(마이그 0028). 인구조사(2026-08-14 덤프)에 **없었던 것이 아니라 그때 존재하지 않았다** — 행 0 은 아티팩트가 아니라 신설 직후의 사실이다. 소유는 등록 시점에 서버가 박는다(common.webhook_outbox.register 가 get_user_group(actor) 를 행에 넣는다) — 요청이 「누구 것으로 만들지」를 말할 수 있는 인자가 없다. 격리 단언은 backend/tests/test_s_webhook_outbox.py::CrossTenantTest 가 네 갈래로 잰다: 남의 구독이 보이지 않는다 · 남의 구독을 못 끈다(거절 + **안 꺼진 것**까지) · 제 것은 꺼진다(양성 대조) · 남의 테넌트 이벤트가 내 주소로 안 나간다. 쓰기 면 둘(등록·해지)은 tests/test_tenant_isolation.WRITE_NO_PROBE 의 kernels.k1_event.subscribe 한 줄이 사유와 함께 가리킨다 / 도달: 라우트가 닿는다 — POST·GET /api/dsm/webhook-subscriptions 와 DELETE …/{subscription_id}. 행 0 이라 P0 문턱(50) 미만이다. D-272 재검사 [2026-09-07 턴 J · 차선 S] — 라우트가 실재하는 것이 맞다. reach 를 고친 근거는 gen_tenant_census.reach_of 의 규칙 그대로다(①자기 pk 경로 → direct_pk · ②pk 는 없으나 이름이 이 모델을 가리키는 경로 → via_parent). 옛 값 no_route 는 **틀린 낱말이 아니라 낡은 사실**이었다 — 그 줄을 쓸 때는 라우트가 없었고, 판정기가 읽던 사진이 2026-08-22(531라우트)라 705라우트에서 생긴 이 경로들을 두 달 동안 못 봤다. ★ 이 줄의 산문은 이미 「라우트가 닿는다」고 적고 있었는데 칸은 no_route 였다 — 한 줄 안에서 두 말이 갈려 있었고, 판정기가 그것을 잡았다. 자리표시자 앞 조각이 이 모델을 가리키므로(gen_tenant_census._is_own_pk ②) direct_pk 다. 재검사 결과 [실측 2026-09-07 · HTTP · gxprobe_s]: DELETE /api/dsm/webhook-subscriptions/999999999 → 404 「그런 구독이 없습니다」 (common.webhook_outbox.revoke 가 assert_scoped 를 먼저 부른다 — 남의 것도 404 라 존재가 안 샌다). GET /api/dsm/webhook-subscriptions → 200 「total 0 · subscriptions 빔」. 목록은 mine() → common.tenant_filters.filter_by_group_field 을 타고, group 이 없는 계정에는 queryset.none() 이다 — 전역 조회로 떨어지는 가지가 없다. 커널 층 격리 단언 4건 통과 (backend/tests/test_s_webhook_outbox.py::CrossTenantTest). ★ 못 쟀다(회색): 행 0 이라 「200 · total 0」이 「내 것만 보였다」인지 「전부인데 전부가 0」인지 HTTP 로는 못 가른다 — 가른 것은 소스이지 실측이 아니다(D-301)"),
    "stream_monitors.Zone":
        ("GAP", "via_parent", "fk", 0,
         "2026-09-01 D-299 로 신설(마이그 0019). 인구조사(2026-08-14 덤프)에 **없었던 것이 아니라 그때 존재하지 않았다** — 행 0 은 아티팩트가 아니라 신설 직후의 사실이다. 소유는 dj-core BaseModel 의 group FK 로 첫 행부터 붙고, 판정 서비스(stream_monitors.services.zones)는 스코프로 좁힌 뒤에만 구역을 돌려준다. 격리 단언은 backend/tests/test_zone_judgment.py 의 test_isolation_another_tenant_cannot_see_our_zones 가 양방향으로 잰다. ★ 「구역을 만드는 공개 면은 아직 없다」는 **더 이상 사실이 아니다** — POST /api/dsm/settings/zones (save_zone) 가 실재한다. 쓰기 IDOR 는 WRITE_PROBES 의 「ZONE.save_zone → 남의 카메라를 내 위험구역에 붙이기」가 이미 잰다(D-290 이 말한 그 등재다). / 도달: 닿는 경로는 GET·POST /api/dsm/settings/zones 둘이고 pk 를 받는 경로는 없다(zone_id 는 본문 인자다) → via_parent. D-272 재검사 [2026-09-07 턴 J · 차선 S] — 라우트가 실재하는 것이 맞다. reach 를 고친 근거는 gen_tenant_census.reach_of 의 규칙 그대로다(①자기 pk 경로 → direct_pk · ②pk 는 없으나 이름이 이 모델을 가리키는 경로 → via_parent). 옛 값 no_route 는 **틀린 낱말이 아니라 낡은 사실**이었다 — 그 줄을 쓸 때는 라우트가 없었고, 판정기가 읽던 사진이 2026-08-22(531라우트)라 705라우트에서 생긴 이 경로들을 두 달 동안 못 봤다. 재검사 결과 [실측 2026-09-07 · HTTP · gxprobe_s]: GET /api/dsm/settings/zones → **403** 「설정 변경 권한이 없는 계정 (감사 #177314)」 — 관문이 HTTP 층에 서 있고 거절이 감사에 남는다. 커널 층 격리 단언 통과: backend/tests/test_zone_judgment.py::ZoneJudgmentTest::test_isolation_another_tenant_cannot_see_our_zones. ★ 못 쟀다(회색): 권한 있는 두 테넌트로 목록을 대조하는 HTTP 시험 — 행 0 · 모든 계정 group=None"),
