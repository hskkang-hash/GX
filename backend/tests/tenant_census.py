...
실측 모수는 **142종**이다 (저장소 관할 · `groups` M2M ∪ `group` FK).
...
**행이 있는 표만** 센다. 그래서 29종이 목록 밖에 있었다 (행 0 이 25종).
행이 0인 것은 "격리가 필요 없다"가 아니라 **"아직 안 썼다"**이다 —
내일 첫 행이 들어오는 순간 그 모델은 무방비로 시작한다.
행이 **있는데도** 빠져 있던 것도 3종 있었다. 그쪽은 0행 아티팩트가 아니라 집계 누락이다.
...
CENSUS: dict[str, tuple[str, str, str, int | None, str]] = {
    ...
    "stream_monitors.DetectionEvent":
        ("GAP", "no_route", "fk", None,
         "인구조사에 없었고 셀 수도 없다 — 저장소 모델 ↔ 운영 스키마 불일치 (P-LOCAL-4) / 도달: 실경로 추적본에 항목이 없고, 전수 라우트에도 이 모델을 가리키는 경로가 없다"),
