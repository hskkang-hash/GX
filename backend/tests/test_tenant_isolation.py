...
WRITE_NO_PROBE: dict[str, str] = {
    "kernels.k1_event.subscribe":
        "★ **면이 열렸다** (2026-09-05 TC · 차선 S · UX-19). 아래에 선등재돼 있던 "
        "`register_webhook_subscription` 과 **같은 자리**여서 두 줄을 이 한 줄로 합쳤다 — "
        "선등재가 「그렇게 되면 두 줄을 한 줄로 고쳐 남긴다」고 적어 둔 그대로다. "
        "왜 probe 가 아니라 사유인가 — **이 함수에는 남의 것을 가리킬 인자가 없다.** "
        "`subscribe(*, scope, webhook_url, filters, signing_key_ref)` 가 전부이고, 행의 "
        "주인은 `get_user_group(scope.require_actor())` 로 **서버가 정한다**. 러너가 "
        "`attempt(test, scope, victim)` 에 넘길 남의 것이 애초에 없다 — 구독은 "
        "시그니처로 막혀 있다(D-281 「시그니처가 1차」). "
        "★ 그리고 **재야 할 자리는 따로 재고 있다**: 이 면의 진짜 교차 쓰기는 해지"
        "(`common.webhook_outbox.revoke`)이고, 그것은 남의 구독 번호를 인자로 받는다. "
        "`tests/test_s_webhook_outbox.py::CrossTenantTest` 가 그것을 잰다 — 거절만이 "
        "아니라 **남의 구독이 꺼지지 않은 것**까지(SEC-07 과 같은 형). 여기 probe 를 "
        "넣으려면 러너에 victim_kind=\"subscription\" 갈래가 먼저 필요하고, 그 갈래는 "
        "이 파일이 모든 차선의 병합 게이트라 이번 턴에 열지 않는다.",
    ...
    # ─────────────────────────────────────────────────────────────────────
    # ★ 2026-09-24 · **2파 선등록** — 차선(S·Q·C)이 이번 파에 여는 쓰기 면 넷.
    #   조율자가 **동시 착수 전에 한 번에** 적는다(지시서 §2 차선 규칙 · §5-1).
    #   규칙은 09-22 선등록과 같다: 면을 여는 커밋이 **이 줄을 지우고 probe 를 넣는다.**
    #   ⚠ 이름은 예정이다 — 차선이 다른 이름으로 열면 그 커밋에서 **이름을 고쳐** 남긴다.
    #
    #   ★ 「회신」은 여기 없다. `kernels.k1_event.reply_from_field` 는 09-04 에 열렸고
    ...
    # ─────────────────────────────────────────────────────────────────────
    # ★ 2026-09-26 · **3파 선등록** — 차선(S·E·C)이 이번 턴에 여는 쓰기 면 다섯.
    #   조율자가 **동시 착수 전에 한 번에** 적는다(09-26 지시서 §2-4).
    #   규칙은 앞의 두 선등록과 같다: 면을 여는 커밋이 **이 줄을 지우고 probe 를 넣는다.**
    #   ⚠ 이름은 예정이다 — 차선이 다른 이름으로 열면 그 커밋에서 **이름을 고쳐** 남긴다.
    #
    #   ★ 왜 다섯인가: 이번 턴의 일감 중 **새로 쓰는 자리**가 이 다섯이다. 나머지는
    #     화면·문서·판정기이고, 화면은 쓰기 면이 아니다(09-24 선등록이 「회신」을
    #     빼면서 적어 둔 그 이유 — 없는 것을 등재하면 대장이 스스로를 부풀린다).
    # ─────────────────────────────────────────────────────────────────────
    "kernels.k2_notify.set_rule_channels":
        "선등재(차선 E · OPS-10 이메일 채널 · P-33) — 아직 없다. 재야 할 것: **남의 "
        "테넌트 규칙의 채널을 내가 바꿀 수 있는가.** 지금 모든 규칙이 `log` 라 사람에게 "
        "가는 경보가 0건이고, 그것을 `email` 로 바꾸는 면이 이번에 열린다. 남의 규칙을 "
        "바꿀 수 있으면 **남의 재난 경보를 끌 수 있다** — 끄는 쪽이 켜는 쪽보다 조용하다.",
    "kernels.k2_notify.save_quiet_hours":
        "선등재(차선 E · 근무 외 차단 · 온보딩 U3 #16) — 아직 없다. 재야 할 것: **남의 "
        "테넌트에 근무 외 시간대를 심을 수 있는가.** 심으면 그 시간 동안 남의 경보가 "
        "조용히 멈춘다. 차단은 **아무 일도 일어나지 않는 모양**이라 사고를 늦게 안다.",
    "apps.dsm.privacy.record_access_request":
        "선등재(차선 S · LAW-07 열람·삭제 청구 접수 · 턴 B) — 아직 없다. 재야 할 것: "
        "**남의 테넌트 사람의 청구를 내가 접수·처리한 것으로 적을 수 있는가.** 개인정보 "
        "청구 기록은 법이 요구하는 증거라, 남의 것에 우리 이름이 남으면 그 증거가 오염된다. "
        "턴 B 일감이지만 이름은 **지금** 적는다 — 면이 먼저 서고 잣대가 뒤따르는 순서를 "
        "만들지 않기 위해서다(P-8).",
    "common.key_rotation.rotate_key":
        "**면이 열렸다** (2026-09-26 · 차선 S · SEC-07). 재는 것은 이미 서 있다 — "
        "`tests/test_s_key_rotation.py::TenantGuardTest` 가 음성(A 관리자가 B 의 키를 "
        "돌리려 하면 `PermissionDenied` + **새 키 행이 안 늘고** 남의 키 수명도 안 바뀐다)과 "
        "양성(제 테넌트 키는 돌아간다)을 함께 잰다. **여기(WRITE_PROBES)로 옮기려면 "
        "`victim_kind=\"apikey\"` 갈래**가 먼저 필요하다 — 지금 러너는 「스트림에 심기」와 "
        "「이벤트 고치기」 둘만 안다. 그 갈래를 만드는 커밋에서 이 줄을 지운다. "
        "★ **이름이 바뀌었다**: 선등재는 `common.inbound_api_key.rotate_key` 였고 실제로는 "
        "`common.key_rotation.rotate_key` 로 열렸다 — 키의 일생(발급·폐기·회전·주기)이 "
        "문지기(`inbound_api_key`)와 다른 일이라 파일을 갈랐다. **지우고 새로 적지 않고 "
        "이름을 고쳐** 남긴다(대장 규약). "
        "★ 사유가 「아직 없다」에서 「다른 파일이 잰다」로 바뀐 것이 이 줄의 이력이다.",

    # ─────────────────────────────────────────────────────────────────────
    # ★ 2026-09-16 턴 S · 차선 U56 — S-16 「알림 받는 사람·채널」(UX-43 · WS-14).
    #
    # ★★ **판별기가 이 둘을 못 봤다. 그래도 적는다** [실측 2026-09-16].
    #   `_writes_to_db` 는 소스에서 `.create(`·`.save(` 를 찾고, 못 찾으면 **한 단계**
    #   위임을 따라간다 — 그 한 단계는 `getattr(module, 이름)` 이라 **모듈 이름공간에
    #   있는 것만** 보인다. 아래 두 함수는 쓰는 쪽(`save_notification_rule` ·
    #   `audit_writer`)을 **함수 안에서 import** 하므로 모듈 속성이 아니고, 그래서
    #   판별기에 안 잡힌다. 즉 이 둘은 **대장이 안 보는 곳에서 늘어난 쓰기 면**이다 —
    #   판별기가 놓쳤다는 이유로 안 적으면 그것이 D-301 이 이름 붙인 그 모양이고,
    #   이 파일의 주석이 09-22·09-04 에 두 번이나 같은 사고를 적어 두었다.
    #   ⚠ 판별기를 넓히는 것은 이 차선의 일이 아니다(이 파일은 모든 차선의 병합
    #     게이트다). 넓히면 `set.update()` 같은 글자까지 쓰기로 잡히는 반대쪽 오류가
    #     생긴다 — 실제로 이번 턴에 `my_notify_reach` 가 그렇게 잡혔고, 그쪽은
    #     **읽기 전용이라 대장에 적지 않고 커널 코드의 글자를 바꿔** 풀었다.
    # ─────────────────────────────────────────────────────────────────────
    "kernels.k2_notify.save_rule":
        "**면이 열렸다** (2026-09-16 · 턴 S · 차선 U56 · UX-43). `save_notification_rule` "
        "**위에** 서는 높은 문턱이다: 저장한 뒤 심각 등급 수신자가 0명이면 "
        "`CriticalWithoutRecipients` 로 **트랜잭션째 되돌린다**(사람의 화면만 이 문으로 "
        "들어오고, 시드·파이프라인은 종전 낮은 문을 그대로 쓴다). "
        "재는 것은 이미 서 있다 — `tests/test_u56_notify_rules.py` 가 거절과 **되돌림**"
        "(행이 안 바뀌는가)을 함께 잰다. 거절만 재고 행을 안 재면 「거절했다고 말하면서 "
        "저장은 된」 상태를 못 본다(D-284). "
        "**여기(WRITE_PROBES)로 옮기려면 `victim_kind=\"rule\"` 갈래**가 먼저 필요하다 — "
        "지금 러너는 「스트림에 심기」와 「이벤트 고치기」 둘만 안다. 바로 위 "
        "`save_notification_rule` 줄이 같은 이유로 같은 자리에서 기다리고 있고, 그 갈래를 "
        "만드는 커밋에서 **두 줄을 함께** 지운다.",
    "kernels.k2_notify.send_test_notification":
        "**면이 열렸다** (2026-09-16 · 턴 S · 차선 U56 · UX-43 「시험 발송」). "
        "쓰는 것은 **감사 한 줄**이다 — `DeliveryRecord` 도 `DetectionEvent` 도 만들지 "
        "않는다(그 표는 F-10 의 30초와 5분 억제가 세는 자리라, 시험 한 건이 끼면 **그 "
        "다음 진짜 경보가 억제로 삼켜진다**). 사건을 안 만드는 것은 P-156 — 측정이 "
        "자기가 재는 표본을 바꾸지 않는다. "
        "재야 할 것: **남의 테넌트 수신자에게 내가 시험 발송을 일으킬 수 있는가.** "
        "지금은 `_group_of(scope)`(→ `require_user_group`)가 문지기이고 남의 것을 가리킬 "
        "인자가 **시그니처에 없다**(D-281 「시그니처가 1차」) — `group=` 을 받지 않는다. "
        "그래서 러너가 `attempt(test, scope, victim)` 에 넘길 남의 것이 애초에 없다"
