...
WRITE_NO_PROBE: dict[str, str] = {
    ...
    "kernels.k1_event.register_webhook_subscription":
        "선등재(차선 S · UX-19 웹훅 구독 · CAP 1.2) — 아직 없다. 재야 할 것: **남의 "
        "테넌트 이벤트를 내 주소로 구독할 수 있는가.** 구독은 읽기처럼 보이지만 "
        "**행을 남기는 쓰기**이고, 한 번 걸어 두면 그 뒤로는 부르지 않아도 계속 나간다. "
        "★ 새 인증 경로를 만들지 않는다(지시서 §4) — F-05 구독 등록 위에 얹는다. "
        "그래서 `kernels.k1_event.subscribe`(구현 전)와 **같은 자리의 이름**이 될 수 "
        "있다. 그렇게 되면 그 커밋에서 두 줄을 한 줄로 고쳐 남긴다.",
