...
★★ 표의 **첫 줄이 칸 수를 정한다** — [실측 2026-09-21 · 턴 AB]
-------------------------------------------------------------
`htmldocx` 는 표를 만들 때 **첫 `<tr>` 의 셀 수로 열을 잡고**, 뒤에 그보다 많은 셀을
가진 줄이 오면 **`IndexError: list index out of range`** 로 죽는다. 그러면
`html_to_docx_bytes` 가 `DocxRenderFailed` 를 내고 **종이가 아예 안 나온다.**

    [실측 · 같은 변환기에 네 모양을 넣어 봄]
      _row · _row · _pair   →  **FAIL**  IndexError            ← 좁은 줄이 먼저
      _pair · _row · _row   →  OK  36,843 B
      _row  · _row · _row   →  OK  36,819 B                    ← 고르면 괜찮다
      _pair · _pair         →  OK  36,830 B

곧 규칙은 하나다: **한 표 안에서 가장 넓은 줄을 맨 위에 두거나, 줄 폭을 고르게 하라.**
`_row` 는 `colspan="3"` 이라 **사람 눈에는** 네 칸이지만 **셀은 둘**이다 —
그 차이가 이 고장의 전부다. 서식을 새로 쓸 때 이 줄을 먼저 읽을 것.

⚠ 이 고장은 **HTML 을 보면 안 보인다.** HTML 로는 멀쩡히 그려지고 DOCX 에서만 죽는다 —
  그래서 서식 시험은 **완성된 DOCX 바이트까지** 가야 한다(아래 「그래도 시험은」 참조).

HWP 가 열게 하려고 지키는 것 넷
-------------------------------
  ① 표는 **단순 격자**만 쓴다(중첩·병합 없음) — HTML 서식이 이미 그 모양이다.
     ★ 그리고 **첫 줄이 가장 넓어야 한다**(바로 위 절).
