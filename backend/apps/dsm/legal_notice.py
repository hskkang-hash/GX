...
----------------------------------------------------------
서식의 안내판에는 「보관 기간 : 기본 30일」이라고 적혀 있다. 그런데 **이 저장소에는
영상 보존 일수를 선언한 자리가 없다** [실측 2026-09-05 · `RETENTION_DAYS` 는
`scripts/` 의 복구 도구에만 있고 제품 설정에는 없다]. 여기서 30을 적으면 그 순간
**안내판이 거짓말을 시작한다** — 게시된 안내판은 되돌릴 수 없고, 그 종이는 법적
효력을 갖는 고지다. 그래서 값이 없으면 `None` 이고 `confirm` 이다. **0 도 30 도 아니다.**
...
def retention_days() -> int | None:
    """보존 일수. **선언된 자리가 없으면 `None`** — 기본값을 지어내지 않는다."""
    for name in RETENTION_SETTING_NAMES:
...
def notice_draft(*, scope) -> dict[str, Any]:
    ...
    board = [
        ...
        _cell("보관 기간", days, AUTO,
              "★ 제품에 영상 보존 일수를 선언한 자리가 없다 — 30일을 적으면 "
              "그 순간 안내판이 거짓말을 시작한다(D-301). 설정이 서면 자동으로 찬다"),
