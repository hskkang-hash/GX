...
PUBLIC_ROUTES: dict[tuple[str, str], str] = {
    ...
    # ★ [D-411 · 2026-09-19] `pair` 는 **제거됐다**(config/urls.py 에서 404).
    #   ⚠ **그런데 줄은 남는다 — 처음엔 뺐다가 되돌렸다.**
    #     빼고 나니 미분류 라우트가 **648 → 649** 로 늘었다(`test_route_tenant_scope`).
    #     라우트는 URLconf 에서 **사라지지 않았다**: `core.urls` 안에 그대로 있고
    #     우리는 앞에 404 문을 세워 가리고 있을 뿐이다. 등재를 지우면 그 자리는
    #     「인증 불요로 판정이 끝난 것」에서 **「아직 아무도 안 본 것」**이 된다 —
    #     빚을 갚은 것이 아니라 **빚을 안 보이게 한 것**이다.
    #     여기 남는 것은 면제가 아니라 **판정**이다. 그래서 사유를 바꿔 남긴다.
    ("POST", "/api/token/pair"):
        "제거된 문 — 404 만 낸다(D-411). 아무것도 내주지 않으므로 인증이 필요 없다. "
        "라우트 자체는 dj-core `core.urls` 안에 살아 있다(§0.4 · 우리가 못 뺀다)",
