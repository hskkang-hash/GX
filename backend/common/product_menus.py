...
PRODUCT_MENUS: tuple[dict, ...] = (
    ...
    },
    {
        ...
        "buckets": (U4,),
        "why": (
            "월간 보고서·별지 제1호 다운로드(UX-25 U4#5·#7). 화면은 턴 T·U 에 섰다 "
            "(`frontend/src/App.tsx` 의 `dsmU24Routes.reports`). "
            "★ [턴 AC] 끝에 붙인다 — 기존 열 줄의 `ordering` 을 안 흔들기 위해서다. "
            "실제로 뜨는 순서는 `roleNav.ts::NAV_ALLOW.U4` 의 표 순서가 정한다 "
            "(`filterNav` 가 그 자리에서 `ordering` 을 다시 매긴다). "
            "⚠ 아이콘 `BsFileEarmarkText` 는 이 DB 에 아직 안 쓰인 이름이다 — "
            "`react-icons/bs` 에 실재하는지 확인하고 넣었다 "
            "[실측 2026-09-22 · gx-fe-build:/app/node_modules/react-icons/bs/index.d.ts]. "
            "⚠ U2 도 이 화면을 본다(P61 「보고서」는 U2·U4 둘 다) — 이 턴은 **U4 만** "
            "건다. U2 쪽은 턴 AD 로 넘긴다."
        ),
    },
    {
        ...
        "buckets": (U4,),
        "why": (
            "감사 대응 이력(UX-25 U4#16). 화면은 턴 T·U 에 섰다 "
            "(`frontend/src/App.tsx` 의 `dsmU24Routes.auditLog`). "
            "★ [턴 AC] 끝에 붙인다 — 위 「보고서」와 같은 이유. "
            "⚠ 아이콘 `BsClockHistory` 도 같은 자리에서 확인했다."
        ),
    },
)
...
P61_NO_SCREEN_YET = {
    ...
    # ★ 「보고서」·「감사 기록」도 **이 표에서 나갔다** (2026-09-22 · 턴 AC).
    #   `/dsm/reports`·`/dsm/audit` 가 턴 T·U 에 섰고, 이번 턴에 U4 표에 걸었다.
    #   아래 P61_SCREEN_ARRIVED 를 본다. 같은 규율 — 지운 것이 아니라 옮긴 것이다.
}
...
P61_SCREEN_ARRIVED = {
    ...
    "보고서(월간 1쪽·HWPX)": (
        "→ `/dsm/reports` 「보고서」 (2026-09-22 턴 AC). **U4 만** 걸었다 — "
        "U2 쪽 접근은 아직 안 걸었다(턴 AD 로 넘긴다)."
    ),
    "감사 기록": "→ `/dsm/audit` 「처리 기록」 (2026-09-22 턴 AC · U4)",
}
