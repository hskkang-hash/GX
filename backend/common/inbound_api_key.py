...
------------------------------
  ...
  ③ 읽기 전용부터            → 쓰기 메서드에는 `inbound_key=True` 를 주지 않는다.
     ⚠ **이 줄은 2026-09-19 까지 거짓이었다** [턴 W · 차선 U1 이 잡고 조율자가 확인].
       원래 「(생성 시 거부)」라고 적혀 있었는데 `__init__` 은 `reason` 만 보고
       **HTTP 메서드는 보지 않는다 — 볼 수도 없다.** 이 인증 객체는 라우트를 꾸밀 때
       만들어지고 그때는 자기가 어느 메서드에 붙는지 모른다.
       **진짜 문지기는 생성자가 아니라 시험**이다:
         `tests/test_f05_inbound_api_key.py::ReadOnlyTest`
         `::test_no_write_route_accepts_an_inbound_key`
       머리말이 「코드가 막는다」고 말하는 동안 아무도 그 코드를 찾지 않았다 —
       **문서가 문지기인 척하면 진짜 문지기를 지우는 사람이 안 보인다.**
