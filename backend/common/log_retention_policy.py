...
#: 근거 조문. **[추정]이다** — 차선 S 가 대조하기 전까지 조문 번호를 적지 않는다.
LEGAL_BASIS = (
    "개인정보 안전성 확보조치 접속기록 보관 기준 **이상** [추정] — "
    "조문 대조는 차선 S 의 몫이다. 이 문자열에 조문 번호가 들어오는 날은 "
    "대조가 끝난 날이고, 그 전에는 이 수로 파기를 켜지 않는다"
)
...
POLICY: dict[str, RetentionClass] = {
    "audit": RetentionClass(
        ...
        why="접속기록은 「누가 무엇을 언제 봤는가」이고, 사고 조사는 대개 몇 달 뒤에 "
            ...
            "⚠ 이 기계에 **심긴 값은 365** 다 — 이 선언과 갈려 있고, 맞추는 것은 "
            "`AdminConfig` 쓰기라 차선 몫이 아니다. 갈렸다는 사실을 판정기가 찍는다",
    ),
