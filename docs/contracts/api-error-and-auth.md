...
**티켓**: W0-18 · **결정**: D-248 · D-212 · **버전**: 1.0 · **고정일**: 2026-08-24
...
| 라우트 선언 | 권한거부 시 실제 응답 | 건수 |
|---|---|---|
| `response=` 없음 | `200` + 위 본문 | 281 |
| `response=List[…]` | **`500`** (pydantic 이 dict 를 거절) | 7 |
| `response=<단일 스키마>` | **`200` + `{}`** — 거부 사실이 **사라진다** | 8 |

세 번째가 특히 위험하다. 클라이언트는 **거부됐다는 것을 알 수 없다.**
(8건은 `report_template` · `checklist_setting` 뿐 — P-W0-18-1 판정 대기)
...
- `response=<단일 스키마>` 8건은 **여전히 200 + `{}`** 다. 거부 정보가 응답이 만들어지기
  전에 소멸해 응답 계층에서 복원할 수 없다.
