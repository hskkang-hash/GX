...
- 커밋: `6c67c20`(코드 72) · `839f33c`(문서 11) · `0c8b957`(창 기록 · P-160 · P-161 · V 증거 · 62) · **④ 이 보고 갱신** — `master` = `turn-q` · **origin push 됨**(0c8b957) · 태그 `pre-turn-t` = 7076b20
...
- 커밋 ③ 자리에서 잡힌 게이트 둘(옳은 빨강 · 고침): `verify_screens` — 대장이 합쳐 쓰이자 옛 항목(204342)이 합친 run_log 의 덮인 단계 시각과 5분 넘게 벌어졌다 → **자기 실행분**(P-157/runs) 시각도 본다(자기시험 +2 · 19/19). `verify_measure_repro` — GA 두 회차가 7/39 ↔ 4/39 로 갈렸다: SEC-21 이 403 → **429** 가 되자 `verify_route_alive.login` 의 「403 이면 61초 기다림」이 눈이 멀어 2회차 F-05 셋(`contract_route_reach` 로그인)이 회색 → 429 + `Retry-After` 도 기다린다 → 6/6 재현.
