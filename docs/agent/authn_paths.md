...
### 9-6. 앞단이 이 헤더를 싣는다 — **브라우저로 실측** (2026-09-06 · 턴 H · 차선 C)

서버 면은 턴 F 에 섰고, 앞판 배선은 턴 G 에 들어갔고, **브라우저가 그 문을 실제로 부른
것은 턴 H 다.** 세 사실은 다르고 날짜도 다르다.

    [실측 2026-09-06 · docs/agent/evidence/P-78/ux24c_remeasure.json]
      /wall#token=<토큰> 으로 열었을 때 브라우저가 보낸 요청 —
        GET /api/dsm/events/queue    X-GX-Wall-Token 실림
        GET /api/dsm/cameras/pulse   X-GX-Wall-Token 실림
      같은 순간 자리 데스크톱(로그인 세션)은 /dsm/queue 를 그대로 보고 있었다.

⚠ **턴 G 에는 이 요청이 0건이었다.** 앞판은 헤더를 실었는데 `CORS_ALLOW_HEADERS` 에
  그 이름이 없어 브라우저가 프리플라이트에서 막았다. 화면은 떴고 콘솔 오류도 0건이라
  **캡처만 봤으면 초록으로 보였을 자리다.** 서버 이름 목록(`config/settings.py`)이
  이 문의 일부라는 사실을 여기 적어 둔다.

⚠ **토큰을 받는 자리는 질의부(`?token=`)가 아니라 조각부(`#token=`)다**
  [P-78 ④ · UX-24c 닫는 조건 ①]. 조각부는 요청에 실리지 않으므로 어떤 앞단도
  접근로그에 적을 수 없다. 옛 주소는 **더는 받지 않는다** — 받으면 그 조건이 계속
  깨진 채로 산다. 앞판 자리: `frontend/src/features/dsm/wallToken.ts`.
