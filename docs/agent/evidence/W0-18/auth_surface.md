...
## 2026-09-26 · SEC-04 로 **24 → 13** (차선 S)

이 문서가 「공개 라우트로 볼 만한 것은 0건」이라 적은 지 한 달 만에 그 24건을 닫았다.
닫은 방법은 조사가 권고한 그대로다 — **라우트마다 `auth=CustomJWTAuth()` 한 줄.**

| | 그때(08-25) | 지금(09-26) [실측] |
|---|---|---|
| 인증 콜백 없음 (전체) | 143 | **72** |
| 그중 `@path_permission` 만 붙은 것 | **24** | **13** |
| 그 13이 사는 곳 | — | **전부 §0.4 금지구역** (terminals 9 · delivery 3 · orders 1) |
| 우리 층 | 28 (09-24 관할 재측정) | **0** |

**남은 13은 0이 되지 않는다.** 우리가 못 고치는 자리이고, 0을 목표로 적으면
영원히 갚지 못하는 빚이 된다. 길이 열린다면 그것은 코드가 아니라 판정이다 —
D-348 이 「금지한 것은 파일 수정이지 길목 차단이 아니다」로 미들웨어 한 겹을 올린 것처럼.

**관문이 섰다는 증거** [실측 2026-09-26 · 익명 호출 재확인]: 6자리 전부 **HTTP 401**
(200 봉투가 아니라 진짜 401) — `/api/stream-monitors/stream-monitors` ·
`/api/stream-monitors/drawing/sessions` · `/api/checklist-setting/categories/` ·
`/api/flight-log/flight-log/` · `/api/partner/partner-callback-mockup/test-websocket-connection` ·
`/api/devices/devices-management`.

★ 눈에 띈 것을 이름으로 적는다: **쓰기 면이 관문 없이 서 있었다** —
스트림 등록 · 외부 스트림 등록/수정 · 녹화 시작/중지 · AI 이중 스트림 시작/중지.
그리고 partner 콜백 모형 4자리는 **익명 POST 로 소켓 알림을 밀어 넣을 수 있는 자리**였다.
