...
> ★ **이 문서 뒤에 한 장이 더 있다 — `P-57_파기.md`** (턴 E · 차선 E).
> 여기 §5 의 「테넌트별 보존기간은 약속할 수 없다」와 §6 의 「객체 바이트를 못 쟀다」가
> 그 문서에서 **둘 다 뒤집혔다**: 카메라를 거쳐 테넌트로 좁히는 길이 있었고,
> 객체저장소를 주고 돌리니 바이트가 사라지는 것도 쟀다(행 0 · 객체 0 · 파기 기록 1).
> 그리고 이 문서가 못 본 것 하나가 거기 있다 — **소프트 삭제된 행은 `objects` 에서도
> 보인다**(dj-core 가 매니저를 덮어썼다). 그 사실이 계량(OPS-16)의 청구서를 좌우한다.
