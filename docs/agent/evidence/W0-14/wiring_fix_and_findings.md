...
> ## ⛔ 이 문서의 §2·§3 누출 수치는 **정정됐다** (2026-08-24)
>
> **원인: 시험 픽스처 오염.** `acting_as()` 가 dj-core 의 `created_by` 자동 채움 경로를
> 덮지 못해, tenant-B 소유로 만들려던 레코드가 **직전 HTTP 요청의 사용자(tenant-A)** 것으로
> 저장됐다. 그러니 A 가 그것을 읽고 지울 수 있는 것은 정상이었다.
>
> 정정된 실측: **격리는 HTTP 레벨에서 버티고 있다.** 남은 실패는 누출이 아니라
> **응답 계약 위반**(Order 상세 500 · Terminal 수정 400 · Terminal 삭제 200 "성공" 거짓말)이다.
>
> → `evidence/W0-14/w0_14c_write_guards_and_correction.md` §0·§1 을 정본으로 읽을 것.
> 아래 §1(배선 결함 4종)과 §4(NO_ROUTE)는 여전히 유효하다.
