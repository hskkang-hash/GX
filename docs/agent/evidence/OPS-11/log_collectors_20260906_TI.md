# OPS-07 — 로그 수집기를 전수로 쟀다 (2026-09-06 22:37:18)
...
| 컨테이너 stdout · postgres | 10m × 5 | 0바이트 | 0바이트 | 드라이버 json-file · 회전 10m × 5 · docker-compose.yml::postgres 에 10m × 5 로 선언돼 있다 (docker run — 이름 대응표) |
| 컨테이너 stdout · redis | 10m × 5 | 5,389바이트 | 129,336바이트 | 드라이버 json-file · 회전 10m × 5 · docker-compose.yml::redis 에 10m × 5 로 선언돼 있다 (docker run — 이름 대응표) |
...
| 컨테이너 stdout · gx-nginx-e | 10m × 5 | 3,469바이트 | 83,256바이트 | 드라이버 json-file · 회전 10m × 5 · compose 에 대응 서비스가 **없다**(docker run — 이름 대응표) — `docker run --log-opt` 로만 걸린다 |
| 컨테이너 stdout · gx-gunicorn-e | 10m × 5 | 15,850바이트 | 380,400바이트 | 드라이버 json-file · 회전 10m × 5 · compose 에 대응 서비스가 **없다**(docker run — 이름 대응표) — `docker run --log-opt` 로만 걸린다 |
...
| 컨테이너 stdout · guardianx-source-mailpit-1 | 10m × 5 | 0바이트 | 0바이트 | 드라이버 json-file · 회전 10m × 5 · docker-compose.yml::mailpit 에 10m × 5 로 선언돼 있다 (compose 라벨) |
| DB 감사 로그 · logger_auditlogs | 무한 | 710행 | 17,040행 | 행 25378개 · 54788096바이트 · 가장 오래된 2026-08-27 13:28:44.947815+00:00 · 워커 1 · ⚠ 보존 365일이 선언돼 있고 워커도 붙었으나 파기 항목이 **DB 주기 표에서 꺼져 있다**(purge-audit-logs-daily=off, ops-audit-purge-daily=off) — 파기 대상 판정(보존 일수 미선언 테넌트 제외)이 서기 전까지 차선 E 가 껐다(P-57 대기). **켜기 전에는 아무것도 안 지워진다** |
...
newest                 2026-09-06 13:37:11.366789+00:00
rows_last_hour         710
retention_days         365
