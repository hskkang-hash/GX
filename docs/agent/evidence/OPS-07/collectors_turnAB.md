# OPS-07 — 로그 수집기를 전수로 쟀다 (2026-09-21 21:02:40)
...
| 컨테이너 stdout · redis | 10m × 5 | 9,643바이트 | 231,432바이트 | 드라이버 json-file · 회전 10m × 5 · docker-compose.yml::redis 에 10m × 5 로 선언돼 있다 (docker run — 이름 대응표) |
...
| 컨테이너 stdout · gx-nginx-e | 10m × 5 | 632바이트 | 15,168바이트 | 드라이버 json-file · 회전 10m × 5 · compose 에 대응 서비스가 **없다**(docker run — 이름 대응표) — `docker run --log-opt` 로만 걸린다 |
| 컨테이너 stdout · gx-gunicorn-e | 10m × 5 | 285바이트 | 6,840바이트 | 드라이버 json-file · 회전 10m × 5 · compose 에 대응 서비스가 **없다**(docker run — 이름 대응표) — `docker run --log-opt` 로만 걸린다 |
| 컨테이너 stdout · gx-celery-e | 10m × 5 | 3,432바이트 | 82,368바이트 | 드라이버 json-file · 회전 10m × 5 · docker-compose.yml::celery 에 10m × 5 로 선언돼 있다 (docker run — 이름 대응표) |
...
| 컨테이너 stdout · guardianx-source-mailpit-1 | 10m × 5 | 531바이트 | 12,744바이트 | 드라이버 json-file · 회전 10m × 5 · docker-compose.yml::mailpit 에 10m × 5 로 선언돼 있다 (compose 라벨) |
| DB 감사 로그 · logger_auditlogs | 무한 | 889행 | 21,336행 | 행 168953개 · 384696320바이트 · 가장 오래된 2026-08-27 13:28:44.947815+00:00 · 워커 1 · ⚠ 보존 365일이 선언돼 있고 워커도 붙었으나 파기 항목이 **DB 주기 표에서 꺼져 있다**(ops-audit-purge-daily=off, purge-audit-logs-daily=off) — 파기 대상 판정(보존 일수 미선언 테넌트 제외)이 서기 전까지 차선 E 가 껐다(P-57 대기). **켜기 전에는 아무것도 안 지워진다** |
...
| `mailpit` | docker-compose.yml | json-file | 10m | 5 |
...
newest                 2026-09-21 12:02:33.459986+00:00
rows_last_hour         889
retention_days         365
purge_beat             {'ops-audit-purge-daily': '<crontab: 40 2 * * * (m/h/dM/MY/d)>'}
file_handlers          []
```
...
## 2. 판정
  ...
  ··X  ② 보존 기간        **보존 기간이 없다**(무한 적재): DB 감사 로그 · logger_auditlogs
  ··   ④ 선언           컨테이너 수집기 전부에 상한이 **선언돼 있다**

갈래 **수집**(①·③·⑤)만 종료코드에 센다 — 나머지 2 판정은 `··` 로 찍었고 **세지 않았다**. 대장의 다른 절이 그 갈래를 센다.
