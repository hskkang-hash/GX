# OPS-07 — 로그 수집기를 전수로 쟀다 (2026-09-05 11:41:59)
...
| 컨테이너 stdout · postgres | 무한 | 0바이트 | 0바이트 | 드라이버 json-file · max-size 없음 · max-file 없음 · compose 에 대응 서비스가 **없다**(docker run — 이름 대응표) — `docker run --log-opt` 로만 걸린다 |
| 컨테이너 stdout · redis | 무한 | 2,243바이트 | 53,832바이트 | 드라이버 json-file · max-size 없음 · max-file 없음 · **선언은 섰다 · 적용은 다음 재기동** — docker-compose.yml::redis 에 10m × 5 로 선언돼 있다 (docker run — 이름 대응표) |
...
| DB 감사 로그 · logger_auditlogs | 90일 (beat ops-audit-purge-daily) | 9행 | 216행 | 행 1402개 · 3571712바이트 · 가장 오래된 2026-08-27 13:28:44.947815+00:00 |
...
newest                 2026-09-05 02:29:17.922106+00:00
rows_last_hour         9
retention_days         90
...
## 2. 판정
  ...
  FAIL ④ 선언           **선언할 자리조차 없는 수집기**: 컨테이너 stdout · postgres, 컨테이너 stdout · gx-fe-build — compose 밖 `docker run` 으로 떴다. **compose 를 고쳐도 닿지 않는다.** 상한은 `--log-opt max-size=10m --log-opt max-file=5` 로만 걸리고, 그 문장은 `docs/agent/RUNBOOK_로컬기동.md` STEP 2A 에 적혀 있다 — 이 빨강은 **다음에 그 컨테이너를 띄우는 사람**이 지운다

판정 **실패**.

**선언은 섰고 적용은 다음 재기동**인 수집기 3개: 컨테이너 stdout · gx-shell, 컨테이너 stdout · redis, 컨테이너 stdout · guardianx-source-minio-1
...
**선언할 자리조차 없는 수집기 2개**: 컨테이너 stdout · postgres, 컨테이너 stdout · gx-fe-build
