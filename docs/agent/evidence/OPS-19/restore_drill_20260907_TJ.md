...
만드는 것: DB 서버 안의 임시 DB `restore_check_20260907065818` 하나. **이 실행이 지운다.**
...
| | |
|---|---|
| 실행 시각 | 2026-09-07T06:58:18+00:00 |
...
```
docker exec postgres psql -U *** -d postgres -c 'CREATE DATABASE "restore_check_20260907065818"'  → rc=0
docker run --rm --network gx-main-network -v gx_backup_vault_e:/backup:ro postgres:latest
    pg_restore --no-owner --no-privileges -h postgres -p 5432 -U postgres -d restore_check_20260907065818 /backup/20260906/db_database_guardianx_20260906T132334Z.dump  → rc=0
docker exec postgres psql -U *** -d postgres -c 'DROP DATABASE "restore_check_20260907065818"'  → rc=0
```
...
**RTO 실측 0.14분** (8.4초) — 임시 DB 만들기부터 지우기까지 벽시계다.
