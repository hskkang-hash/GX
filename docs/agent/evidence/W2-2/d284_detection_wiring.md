...
```
시험   133 passed · 0 skipped    (직전 118 · 0 · 그 앞 111 · 1 skipped)
...
1. **스냅샷 1장을 MinIO 에 올리지 않았다** (W2-2 spec 이 요구한다).
   `snapshot_path` 는 지금 **빈 문자열**로 넘어간다. **가짜 경로를 넣지 않았다** —
   있지도 않은 객체를 가리키는 경로는 "저장했다"는 거짓말이고, 그것이 D-284 가 이름 붙인
   조용한 성공이다. 외부 저장소 호출이므로 타임아웃(W0-17 · C-3.3)이 함께 걸린다.
