 ...
 *
 * ★★ [2026-09-16 · 턴 S] **구독이 섰다.** 앞판 주석은 「`push-subscriptions` 문이
 *   아직 없다」였는데, 그 문이 이번 턴에 섰다(`apps/dsm/api_u3.py`) — 등록·목록·
 *   해지·시험 발송 넷이다. 그래서 이 파일은 이제 **진짜로 받는 자리**다.
 *
 *   서버가 싣는 모양도 이제 정해졌다(`kernels/k2_notify/channels.py::WebPushChannel`):
 *       `{"title": …, "body": …, "url": "/m/inbox"}`
 *   ⚠ 그래도 아래 `readPayload` 의 **자리표는 지우지 않는다.** 푸시 서비스가 본문
 *     없는 알림을 흘려보내는 갈래가 있고(용량 초과·중계 실패), 그때 빈 알림이 뜨면
 *     사람은 「무슨 일이 났는데 뭔지 모르는」 상태가 된다. 무엇이 와도 죽지 않는다.
 *
