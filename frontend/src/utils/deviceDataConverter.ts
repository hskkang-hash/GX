...
const formatDate = (value: any, dateFormat?: string) => {
  ...
  if (dateFormat) {
    const parsed = dayjs(value, dateFormat, true);
...
export const convertDeviceData = (
  sourceData: any,
  ...
  const manufacturer_information = {
    ...
    country_of_origin_id: data.manufacturer_information?.country_of_origin_id
      ...
      : null,
    production_date: data.manufacturer_information?.production_date ? convertDateFormatToYYYYMMDD(data.manufacturer_information?.production_date) : null,
    validity_period_from: data.manufacturer_information?.validity_period_from ? convertDateFormatToYYYYMMDD(data.manufacturer_information?.validity_period_from) : null,
    validity_period_to: data.manufacturer_information?.validity_period_to ? convertDateFormatToYYYYMMDD(data.manufacturer_information?.validity_period_to) : null,
  ...
  const convertedData = isRoleSuperuser
    ? {
      ...(data?.status_id && { status_id: data?.status_id || null }),
      unit_id: data?.unit_id?.value?.value || null,
      library_id: data?.library_id?.value?.value || null,
      terminal_id: data?.terminal_id?.value?.value || null,
      serial_number: data.serial_number || '',
      name: data.name || '',
      color: data.color || '',
      group_id: data?.group?.value?.value || null,
...
export const convertDeviceDataForEditHaveLibrary = (
  sourceData: any,
  primaryData: any,
  isRoleSuperuser: boolean = false,
) => {
  console.log('primaryData_convertDeviceDataForEditHaveLibrary', primaryData);
  if (Object.keys(sourceData).length === 0) return null;
  const resultData = {
    data: {
      serial_number: primaryData?.serial_number || '', // Số serial (bắt buộc)
      name: sourceData?.name || '', // Tên thiết bị (bắt buộc)
      active: primaryData?.active || true, // Trạng thái hoạt động (mặc định: true)
      main_type_id: primaryData?.main_type_id || null, // ID của main_type (không bắt buộc)
      sub_type: primaryData?.sub_type || '', // Loại phụ (mặc định: "standard")
      note: primaryData?.note || '', // Ghi chú (không bắt buộc)
      color: primaryData?.color || '#ff0000',
      group:
        isRoleSuperuser && sourceData?.group__id
          ? {
            value: {
      ...
      unit_id: {
        value: {
          label: primaryData?.unit_id || null,
          value: primaryData?.unit_id || null,
        },
        type: 'select',
      },
      library_id: {
        value: {
          label: primaryData?.library_name || null,
          value: primaryData?.library_id || null,
        },
        type: 'select',
      },
      terminal_id: {
        value: {
          label: primaryData?.terminal__name || null,
          value: primaryData?.terminal_id || null,
        },
        type: 'select',
      },
      dimensions: {
        ...
        frame_size: {
          length: {
            ...
            unit: sourceData?.dimensions?.frame_size?.unit || 'mm',
          },
          width: {
        ...
        maximum_takeoff_weight: {
          ...
          unit: sourceData?.dimensions?.maximum_takeoff_weight?.unit || 'kg',
        }, // Trọng lượng cất cánh tối đa
        payload_capacity: {
          value: sourceData?.dimensions?.payload_capacity?.value || null,
          unit: sourceData?.dimensions?.payload_capacity?.unit || 'kg',
        }, // Khả năng chở hàng
        empty_weight: {
          value: sourceData?.dimensions?.empty_weight?.value || null,
          unit: sourceData?.dimensions?.empty_weight?.unit || 'kg',
        }, // Trọng lượng rỗng
        frame_class_id: {
          value: {
      ...
      propulsion_system: {
        ...
        flight_time: {
          ...
          unit: sourceData?.propulsion_system?.flight_time?.unit || 'minutes',
        }, // Thời gian bay
        charging_time: {
          value: sourceData?.propulsion_system?.charging_time?.value || null,
          unit: sourceData?.propulsion_system?.charging_time?.unit || 'minutes',
        }, // Thời gian sạc
        motor_power: {
          value: sourceData?.propulsion_system?.motor_power?.value || null,
          unit: sourceData?.propulsion_system?.motor_power?.unit || 'kW',
        }, // Công suất động cơ
        propeller_size: {
          value: sourceData?.propulsion_system?.propeller_size?.value || null,
          unit: sourceData?.propulsion_system?.propeller_size?.unit || 'inch',
        }, // Kích thước cánh quạt
        battery_capacity: {
          value: sourceData?.propulsion_system?.battery_capacity?.value || null,
          unit: sourceData?.propulsion_system?.battery_capacity?.unit || 'mAh',
        }, // Dung lượng pin
      },
      ...
      flight_performance: {
        ...
        maximum_speed: {
          ...
          unit: sourceData?.flight_performance?.maximum_speed?.unit || 'km/h',
        }, // Tốc độ tối đa
        cruise_speed: {
          value: sourceData?.flight_performance?.cruise_speed?.value || null,
          unit: sourceData?.flight_performance?.cruise_speed?.unit || 'km/h',
        }, // Tốc độ hành trình
        maximum_altitude: {
          value:
            sourceData?.flight_performance?.maximum_altitude?.value || null,
          unit: sourceData?.flight_performance?.maximum_altitude?.unit || 'm',
        }, // Độ cao tối đa
        operating_altitude: {
          from: {
            value:
            ...
            unit:
              sourceData?.flight_performance?.operating_altitude?.unit || 'm',
          },
          to: {
            value:
        ...
        maximum_range: {
          ...
          unit: sourceData?.flight_performance?.maximum_range?.unit || 'km',
        }, // Phạm vi tối đa
        wind_resistance: {
          value: sourceData?.flight_performance?.wind_resistance?.value || null,
          unit: sourceData?.flight_performance?.wind_resistance?.unit || 'km/h',
        }, // Khả năng chống gió
      },
      ...
      navigation_control: {
        ...
        gps_accuracy: {
          ...
          unit: sourceData?.navigation_control?.gps_accuracy?.unit || 'm',
          icon_device:
      ...
      radio_communication: {
        ...
        frequency: {
          ...
          unit: sourceData?.radio_communication?.frequency?.unit || 'GHz',
        }, // Tần số
        range: {
          value: sourceData?.radio_communication?.range?.value || null,
          unit: sourceData?.radio_communication?.range?.unit || 'km',
        }, // Phạm vi
      },
      ...
      manufacturer_information: {
        ...
        validity_period_from:
          sourceData?.manufacturer_information?.validity_period_from || '',
        validity_period_to:
          sourceData?.manufacturer_information?.validity_period_to || '',
        current_status:
        ...
        production_date:
          sourceData?.manufacturer_information?.production_date || null,
        insurance_status:
          sourceData?.manufacturer_information?.insurance_status || false,
        registration_number:
          sourceData?.manufacturer_information?.registration_number || '',
      },
      ...
      environmental_specification: {
        ...
        humidity: {
          ...
          unit: sourceData?.environmental_specification?.humidity?.unit || '%',
        }, // Độ ẩm
        precipitation:
        ...
        wind_speed: {
          ...
          unit:
            sourceData?.environmental_specification?.wind_speed?.unit || 'km/h',
        }, // Tốc độ gió
        noise_takeoff: {
          value:
        ...
        noise_cruise: {
          value:
            ...
            null,
          unit:
            sourceData?.environmental_specification?.noise_cruise?.unit || 'dB',
        }, // Tiếng ồn khi bay
        noise_landing: {
          value:
      ...
      cargo_compartments: {
        ...
        dimensions: {
          length: {
            ...
            unit: sourceData?.cargo_compartments?.dimensions?.unit || 'mm',
          },
          width: {
        ...
        weight_capacity: {
          ...
          unit: sourceData?.cargo_compartments?.weight_capacity?.unit || 'kg',
    ...
    avatar: primaryData?.avatar || null,
    files: sourceData?.file_attachments || [],
  };
  return resultData;
};
interface ConvertDeviceParams {
  sourceData: any;
  isRoleSuperuser: boolean;
  unitPreferences: any;
  converRawDateToDateFormat: any;
}
export const convertDeviceDataForEdit = ({
  sourceData,
  isRoleSuperuser,
  unitPreferences,
  ...
  const resultData = {
    data: {
      status_id: sourceData?.status_id || null,
      serial_number: sourceData?.serial_number || '', // Số serial (bắt buộc)
      name: sourceData?.name || '', // Tên thiết bị (bắt buộc)
      active: sourceData?.active || true, // Trạng thái hoạt động (mặc định: true)
      main_type_id: sourceData?.main_type__id || null, // ID của main_type (không bắt buộc)
      sub_type: sourceData?.sub_type || '', // Loại phụ (mặc định: "standard")
      note: sourceData?.note || '', // Ghi chú (không bắt buộc)
      color: sourceData?.color || '#ff0000',
      group:
        isRoleSuperuser && sourceData?.group__id
          ? {
            value: {
      ...
      terminal_id: {
        value: {
          label: sourceData?.terminal__name || null,
          value: sourceData?.terminal_id || null,
        },
        type: 'select',
      },
      unit_id: {
        value: {
          label: sourceData?.unit_id || null,
          value: sourceData?.unit_id || null,
        },
        type: 'select',
      },
      library_id: {
        value: {
          label: sourceData?.library__name || null,
          value: sourceData?.library_id || null,
        },
        type: 'select',
      },
      dimensions: {
      ...
      manufacturer_information: {
        ...
        policy_number:
          sourceData?.manufacturer_information?.policy_number || '',
        validity_period_from: sourceData?.manufacturer_information?.validity_period_from ? converRawDateToDateFormat(sourceData?.manufacturer_information?.validity_period_from) : null,
        validity_period_to: sourceData?.manufacturer_information?.validity_period_to ? converRawDateToDateFormat(sourceData?.manufacturer_information?.validity_period_to) : null,
        production_date: sourceData?.manufacturer_information?.production_date ? converRawDateToDateFormat(sourceData?.manufacturer_information?.production_date) : null,
        current_status:
          sourceData?.manufacturer_information?.current_status || 'active',
        serial_number:
          sourceData?.manufacturer_information?.serial_number || '',
