...
/**
 * 화면에 뜨는 글자. **값이 없으면 빈 글자다 — 「미등록」이라 적지 않는다.**
 *
 * ★ [턴 AA · 2026-09-21 · 세종 실사용 점검 §3 ①] 위 머리말의 판단은 그대로 옳다 —
 *   **빈 값에 그럴듯한 번호를 채우면 막다른 골목**이다. 그런데 세종이 고객 자리에
 *   앉아 본 것은 다른 자리였다:
 *
 *     「**장애 신고 창구 미등록**」은 우리 회색을 **고객 발치에 적어 둔 것**이다.
 *
 *   로그인 화면 하단에 그 글자가 있으면, 처음 온 재난안전과 담당자는 제품이 아직
 *   덜 됐다고 읽는다. **우리가 아직 못 정한 것을 고객에게 보고하게 할 이유가 없다.**
 *
 *   그래서 셋째 길을 쓴다 — 지어내지도, 「미등록」이라 적지도 않고, **안 그린다.**
 *   값이 들어오는 날 줄이 저절로 선다(`__GX_SUPPORT__` 를 빌드가 채운다).
 *   ⚠ 이것은 **회색을 숨기는 것이 아니다.** 회색은 대장과 보고에 그대로 남고,
 *     사라지는 것은 **고객 화면의 글자**뿐이다.
 */
export const SUPPORT_LABEL = GX_SUPPORT ? `장애 신고 ${GX_SUPPORT}` : '';
