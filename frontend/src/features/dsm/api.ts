...
export const dsmSystemEndpoint = {
   ...
   *
   * ★ [턴 W · 차선 U56] **이 문이 섰다.** 그 전까지 이 경로는 404 였고(넉 달 ·
   *   U5-BACKUP-404), 화면은 그 404 를 「미선언」이 아니라 **「백엔드 신호 대기」**
   *   회색으로 그렸다 — 둘은 다른 사실이기 때문이다(미선언은 「아무도 안 정했다」,
   *   신호 대기는 「우리가 못 읽는다」). 이제 회색이 아니라 **선언 자체**가 온다.
   *   404 갈래는 **지우지 않는다** — 서버가 옛 판이면 그 사실이 다시 회색으로 온다.
   * ⚠ 경로 글자를 바꾸지 마라. 이 문자열이 서버 라우트와 **한 글자라도** 다르면
   *   404 가 그대로 돌아오고, 화면은 그것을 조용한 회색으로 그린다(빨강이 아니다).
   */
...
export interface DsmStorageDeclaration {
   ...
   * 「설정은 기본값이 아니라 선언이다」 — 값만 보이면 `50 GB` 가 누가 정한 수인지
   * 화면에서 사라진다. 서버가 안 보내면 빈 문자열이고 화면은 지어내지 않는다.
   */
