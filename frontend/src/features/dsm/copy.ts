...
export const WIRING_WAITING_NOTE =
  // (턴 T · 병합) 둘째 문장 「아래 한 줄은 … 검수용 예시」를 뗐다 — U1 이 시드 예시 블록을
  // 지우고 카드를 실자료로 이었으므로 그 문장은 거짓이 됐다(D-284 의 반대 방향).
...
/* ════════════════════════════════════════════════════════════════════════
 * U1 — 턴 AA (2026-09-21 · P-221 · 세종 실사용 점검)
 * ════════════════════════════════════════════════════════════════════════
 *
 * ★ **끝에 붙인다**(위 절들과 같은 관례). 사전 정본(`docs/design/GX-COPY_v1.md`)에
 *   같은 절을 올릴 것을 조율자에게 쪽지로 청했다
 *   (`docs/agent/checkpoints/turn-aa/조율자.inbox/U1.md`) — 이 파일은 사전의
 *   **코드 쪽 반쪽**이다.
 *
 * ★ 이 절의 두 문구는 같은 불변에서 나왔다: **「정직한 회색을 고객 말로」.**
 *   「377시간 27분」도 「영상 구간 없음」도 **참말**이다. 그런데 고객은 앞의 것을
 *   「이 시스템은 방치돼 있다」로, 뒤의 것을 「CCTV 인데 영상이 없다」로 읽는다.
 *   참말을 고쳐 적는 것이 아니라 **같은 참말을 고객의 말로** 적는다.
 */

/**
 * 대응 시계가 **하루를 넘겼을 때**의 말 (③).
 *
 * 세종 2026-09-21 [고객 자리 실측]: 큐의 「가장 급한 하나」가 15일 전 사건이고
 * 대응 시계가 **「377시간 27분」 빨강**이었다. *「제품은 옳다(오래 열린 것이 급하다).
 * 그러나 고객 첫날 화면에 377시간은 「이 시스템은 방치돼 있다」로 읽힌다.」*
 *
 * ★ **시간을 감추지 않는다.** 날로 바꿔 읽기 쉽게 하고, **다음 손**을 같은 줄에
 *   붙인다 — 하루가 넘은 사건에 남은 손은 대응이 아니라 **종결 검토**다.
 *   정확한 초는 없어지지 않는다: 툴팁과 스크린리더 라벨이 그대로 싣는다.
 */
export const ELAPSED_REVIEW_CLOSE = '종결 검토';

/** 「15일 경과 · 종결 검토」. 날 수는 **센 수다 — 손으로 적지 않는다.** */
export function elapsedDaysPhrase(days: number): string {
  return `${days}일 경과 · ${ELAPSED_REVIEW_CLOSE}`;
}

/**
 * 「영상 구간 없음」을 **원인 + 다음 손**으로 (⑤).
 *
 * 세종 2026-09-21: *「고객에게 「영상 구간 없음」은 「CCTV 인데 영상이 없다」이다.」*
 * 없는 것은 영상이 아니라 **설정**이다 — 그 카메라에 사건 구간 저장이 아직 안 켜졌다.
 *
 * ★ **원인과 다음 손을 같은 줄에.** 「없음」은 원인도 다음 손도 말하지 않아서,
 *   읽은 사람이 할 수 있는 일이 「고장 신고」밖에 없다.
 * ★ 괄호 안은 **실재하는 자리**다(관리자 → 카메라 등록). 실제 값이 없는 길을 적으면
 *   그것은 안내가 아니라 미로다(불변 「실제 값이 없는 문구는 비표시」).
 */
export const CLIP_MISSING_REASON =
  '이 카메라는 사건 구간 저장이 아직 켜지지 않았습니다 (관리자 → 카메라 등록)';

/** 영상 구간이 **있을 때**. 종전 글자 그대로 — 있는 것에 설명을 붙이지 않는다. */
export const CLIP_PRESENT = '있음';

/**
 * 종결 확인 칸이 **0건일 때 한 줄로 접힌다** (④).
 *
 * 세종 2026-09-21: *「조치를 마쳤다고 알려 온 사건이 없습니다」가 화면 절반을
 * 차지한다.* 0 건인 칸이 제목 · 테두리 · 빈 본문으로 한 화면을 먹으면, 사람은
 * **있는 일감보다 없는 일감을 먼저 본다.**
 *
 * ★ **지우지 않는다. 접는다.** 칸을 없애면 「조치 완료 회신이 오는 자리가 있다」는
 *   사실이 화면에서 사라지고, 회신이 처음 왔을 때 아무도 그 자리를 모른다.
 *   그래서 한 줄은 남고, **0 이라고 적는다**(0 건인 갈래도 지우지 않는다).
 * ★ 접히는 것은 **진짜 0 일 때뿐**이다. 「배선 대기」(회신은 오는데 종류 표시가
 *   0 건)는 **다른 사실**이고 접지 않는다 — 접으면 「없음」과 「못 읽음」이 한 줄이 된다.
 */
export const CLOSE_CONFIRM_EMPTY_ONE_LINE =
  '종결 확인 0건 — 현장이 모바일에서 「조치 완료」를 보내면 여기에 뜹니다.';

/**
 * 되돌림(「종결 → 조치 중」) 확인 창의 안내 한 줄 (P-224 · 턴 AA · 조율자 쪽지).
 *
 * 종전 문안은 *「사유가 비면 **서버가 400 으로 거절합니다**」* 였다. 참말이고 **고객 말이
 * 아니다** — 400 은 우리 저장소의 낱말이고, 읽은 사람이 할 수 있는 일을 하나도 말하지
 * 않는다. 차선 Q 의 `verify_error_body` 가 이 줄을 잡았고, U3 가 모바일에서 같은 자리를
 * 이미 고쳤다(「서버가 404」 → 「저장된 구간이 없습니다」).
 *
 * ★ **원인과 다음 손을 같은 줄에.** 원인은 「사유가 비어 있다」이고 다음 손은 「한 줄
 *   적는다」다. 상태 숫자를 뺀다고 사실이 없어지지 않는다 — 거절은 그대로 일어나고,
 *   거절당한 사람이 **무엇을 하면 되는지**만 더해졌다.
 * ★ 누가 할 수 있는지(관제팀장)는 남긴다. 그것을 빼면 권한이 없는 사람이 사유를
 *   적고 나서야 막히고, 그때는 이미 쓴 글이 버려진다.
 */
export const RESPONSE_BACKWARD_NEEDS_REASON =
  '되돌림은 「종결 → 조치 중」 하나뿐이고 관제팀장만 할 수 있습니다. '
  + '되돌리는 사유를 한 줄 적어 주십시오 — 비어 있으면 되돌려지지 않습니다.';

/* ═══════════════════════════════════════════════════════════════════════════
 * 감사 **채널 이름** 사전 — 턴 AA · 차선 U24 (P-221 이 이름으로 부른 빈 자리)
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * 무엇이 문제였나 [실측 2026-09-21 · 턴 Z]
 * ---------------------------------------
 * 대표 결정 ⑤ 로 감사 화면이 읽는 채널이 **2 에서 15** 가 됐다. 그전까지 화면은 이름
 * 둘만 우리말로 바꾸고 나머지는 서버가 준 글자를 그대로 적었는데, 그 「나머지」가
 * 0 건이라 아무도 안 봤다. 넓힌 뒤 재난안전과 담당관 한 사람의 화면에서만
 * **368 행**이 그 길로 떨어진다 — 감사 화면이 `guardianx.dsm.response` 라고 말한다.
 * 그것은 **우리 폴더 구조를 읽어 주는 것**이고 사전 §4 가 금한 자리다.
 *
 * ★ 표는 턴 Z 에 `AuditLog.tsx` 안에 섰다. 이 턴에 **여기로 옮긴다** — 판정기가
 *   표시명 사전을 찾는 자리가 이 파일이고(`verify_ui_copy.aa_display_dicts`),
 *   화면 파일 안에 있으면 **사전이 있는데도 「없다」로 읽힌다.** 두 벌을 만들지 않으려고
 *   옮기는 것이지 새로 짓는 것이 아니다 — 낱말은 한 글자도 안 바뀌었다.
 *
 * ★★ **모르는 채널을 그럴듯한 우리말로 지어내지 않는다.** 표에 없는 이름은
 *   「우리말 이름 없음」이고 원래 이름은 `title` 에만 둔다(역할 표시명 사전과 같은 규율).
 *   지어내면 그 한국어가 **틀렸다는 것이 안 보인다.**
 */
export const CHANNEL_LABEL: Readonly<Record<string, string>> = {
  ...
  //: ⚠ 꾸며 적지 않는다. 시험이 운영 감사표에 남긴 행이고, 「시험이 남긴 행」이라고
  //:   적는 것이 이 화면이 할 수 있는 유일한 참말이다.
  'guardianx.test.law08_race': '시험이 남긴 행',
};

/** 표시명이 없을 때 화면에 뜨는 말. **기계 이름이 아니다.** */
export const CHANNEL_NAME_UNKNOWN = '우리말 이름 없음';

/**
 * 채널 이름 하나. **모르면 「우리말 이름 없음」** — 서버가 준 글자를 그대로 내보내지 않는다.
 * 앞뒤 공백만 눕힌다. 접두 일치·유사도는 **하지 않는다** — 그 순간 모르는 채널이
 * 아는 채널의 이름으로 읽힌다.
 */
export function channelLabel(name: string | null | undefined): string {
  return CHANNEL_LABEL[String(name ?? '').trim()] ?? CHANNEL_NAME_UNKNOWN;
}

/** 우리말 이름을 아는 채널인가. 화면이 「우리말 이름 없음」을 흐리게 그릴 때 쓴다. */
