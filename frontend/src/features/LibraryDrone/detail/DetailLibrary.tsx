...
const DetailLibrary = () => {
  ...
  const fetchData = async (id: number) => {
    const { success, data, message } = await getLibraryById(id);
    if (success) {
      setDataDetail(data);
    }
    if (!success) {
  ...
  const handlePreview = (file: any) => {
    ...
    setPreviewImage(import.meta.env.VITE_API_URL + file?.file_url);
    setPreviewOpen(true);
  };
      ...
      detailsSection: [
        {
          ...
          list: [
            ...
            },
            {
              ...
              value: dayjs(
                dataDetail?.manufacturer_information?.production_date,
              ).format('MM-DD-YYYY'),
            },
            {
              label: t('Note'),
  ...
  return (
    <>
      <CustomBreadcrumb
        items={[{ url: '/library' }, { text: 'Detailed Information' }]}
        buttons={[
          <CustomBtn
            variant="outline"
            color="primary"
