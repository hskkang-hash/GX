...
import {
  ...
  LOG_CHANNEL_NOTE,
  logOnlyNextHand,
  //: ⑤ [턴 AA] `OUTCOME_LOG_ONLY`(데스크 표의 「기록만 · 사람에게 안 갔습니다」)는
  //:   **안 쓴다.** 이 카드는 배지(「기록만」)와 결과 줄(`OUTCOME_NOT_DELIVERED`)로
  //:   나누어 적는다 — 좁은 화면에서 한 줄에 다 넣으면 시각이 밀려 안 보인다.
  //: ★ [턴 AB] 그 둘도 이제 **`deliveryOutcome.tsx` 에서 온다.** 이 파일은 말을
  //:   **하나도 들고 있지 않다** — 정본은 한 파일이고 여기는 그리기만 한다.
  OUTCOME_NOT_DELIVERED,
  reachesAPerson,
  RECIPIENT_NONE_SUFFIX,
...
/**
 * ⑤ [턴 AA → **턴 AB** · U3] **이 화면의 말은 이제 여기 없다.**
 *
 * 턴 AA 에 이 자리에는 상수 둘(`LOG_ONLY_CHANNEL_LABEL` · `OUTCOME_NOT_DELIVERED`)과
 * 함수 둘(`channelDisplayLabel` · `logOnlyNextHand`)이 **제 몸으로** 서 있었고,
 * 데스크(`deliveryOutcome.tsx`)에는 **같은 사실을 말하는 다른 글자**가 있었다.
 * 그 갈림 때문에 한 턴 안에 말이 두 번 바뀌었고(「훈련」 → 「기록만」) 한쪽이 옛말로
 * 남을 뻔했다 — D-212 가 말하는 바로 그 모양이다.
 *
 * ★★ **턴 AB 에 넷 전부를 `features/dsm/deliveryOutcome.tsx` 로 옮겼다.**
 *   이 파일은 이제 **말을 하나도 들고 있지 않다.** 위 import 가 전부이고,
 *   「사람에게 안 갔다」를 **판정하는 식도 그 파일의 `reachesAPerson` 하나**다.
 *   ⇒ 낱말을 고치려면 **그 파일 한 곳**을 고친다. 여기를 고치면 안 된다.
 *
 * ★ 무엇을 **안 바꿨나**(옮기면서 글자가 바뀌면 그건 옮긴 것이 아니다):
 *     · 배지 글자 「기록만」 · 결과 줄 「사람에게 안 갔습니다 — 기록에만 남았습니다」
 *     · 카드 머리 한 줄(「… 관리자에게 알림 채널 등록을 요청하십시오」)
 *     · 색과 표식(주황 · `▲`) — 초록으로 그리면 F-10 이 **거짓으로 달성**된다
 *     · 계약 값 `data-gx-channel` — 여전히 `log` 원래 값 그대로다
 *   옮긴 뒤 **번들에서 같은 글자를 다시 찾아** 대 봤다(보고의 세 갈래 표).
 *
 * ⚠ 왜 「기록만」이 「훈련」이 아닌지, 왜 카드마다 한 번인지 — **사유는 전부
 *   정본 파일 머리말**에 있다. 여기 두 벌로 적지 않는다. 사유가 두 곳에 있으면
 *   다음 사람이 어느 쪽이 지금 말인지 못 가린다.
 */
                            ...
                            */
                            ...
                            <Text type="secondary" style={{ fontSize: 12 }}>
                              {/* ★ 이 라우트는 이름이 아니라 `recipient_id` 만 낸다 —
                                  ...
                                  ★ [턴 AB] 그 꼬리말도 **정본에서 온다**
                                  (`RECIPIENT_NONE_SUFFIX`) — 데스크 표의
                                  「(수신자 없음)」 과 한 파일에서 나란히 산다. */}
                              발송 {shortAbsolute(d.sent_at)}
                              {d.recipient_id === null ? ` · ${RECIPIENT_NONE_SUFFIX}` : ''}
                            </Text>
                          )}
                        </Space>
                      );
