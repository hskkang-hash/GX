...
import { getPhoneMaskTypeByLanguage, useDebounce } from '@/utils/utils';
...
const ShippingInfomation = () => {
  ...
  // Watch sender location_id to extract only the ID value
  const senderLocationId = watch('data.sender.location_id.value');
  const senderLocationIdValue = typeof senderLocationId === 'object' && senderLocationId !== null 
  ...
  const memoizedGetTerminalsWithAddress = useMemo(
    () => getTerminals({ address: recipientAddress, pickup_location_id: senderLocationIdValue }),
    [getTerminals, recipientAddress, senderLocationIdValue],
  );
          ...
          <CustomSearchMapUnified
            ...
            onSelect={(value, option) => {
              ...
              if (value) {
                ...
                trigger('data.recipient.full_address' as any);
              }
            }}
            onClear={() => {
            ...
            onError={errors?.data?.recipient?.full_address?.message as string}
