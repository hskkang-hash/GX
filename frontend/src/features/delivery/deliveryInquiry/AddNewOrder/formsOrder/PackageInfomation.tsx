...
const PackageInfomation = () => {
  ...
  console.log('configSystem', configSystem);
  const activeAddMorePackages =
    configSystem?.['Add more packages']?.value ?? false;
    ...
    const subscription = watch((_, { name }) => {
      ...
      if (isPackagingField) {
        ...
        if (hasParamsChanged) {
          ...
          // Only set if value is not already null/empty
          if (
            currentValue !== null &&
            currentValue !== '' &&
            currentValue !== undefined
          ) {
            setValue(`data.package.${index}.packaging.value`, null, {
