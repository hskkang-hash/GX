...
const DetailOrder = () => {
  ...
  const orderDetailAwaitShipment =
    dataDetail?.status__code == 'awaiting_shipment';
  const cashPayment = dataDetail?.payment_method__code == 'cash';

  const awaitingPaymentFail =
    dataDetail?.payment_status == 'failed' &&
    dataDetail?.status__code == 'awaiting_payment';
  const [showModalRefund, setShowModalRefund] = useState<boolean>(false);
  const [showModalRefundCash, setShowModalRefundCash] =
    useState<boolean>(false);
  const [cancelReorder, setCancelReorder] = useState<boolean>(false);
  const { getDetailOrder, cancelOrder, getBanks, refundCash } = useAPI();
  ...
  const senderAndRecipientData = [
    {
      ...
      list_item: [
        ...
        },
        {
          ...
          value: dataDetail?.pickup_location,
        },
        {
          name: t('Note'),
  ...
  const orderDetail = [
    { id: 1, name: 'Order ID', value: dataDetail?.order_code, label: 'all' },
    {
      ...
      name: 'Status',
      styled: true,
      value: formatStatus(dataDetail?.status__code, theme, t),
      label: 'all',
    },
    {
      id: 3,
  ...
  const pageType = dataDetail?.status__code as PageType;
  ...
  const handleCancelOrder = async (reason: any, isReorder: boolean) => {
    ...
    console.log('isReorder11', isReorder);
    ...
    <>
      <FormProvider {...methods}>
        <form className="form-add-new-device">
          <CustomBreadcrumb
            items={[{ url: '/delivery-inquiry' }, { text: t('Order Detail') }]}
            buttons={[
          ...
          <Main>
            {!orderDetailAwaitShipment && (
              ...
              >
                ...
                >
                      ...
                      {activePayment && (
                        ...
                        >
                          ...
                          <div className="d-flex justify-content-between header-title pb-0">
                            ...
                            <span>
                              {t(
                                dataDetail?.payment_details?.payment_method ||
                                '-',
                              )}
                            </span>
                          </div>
                  ...
                  {(pageType == 'returned' || pageType == 'delivered') && (
                    <ExpanDropDown
                      label={t('History')}
            ...
            {/* CASH PAYMENT */}
            {pageType == 'pending_confirmation' && showModalRefundCash && (
              <CustomModal1
                title={t('Refund')}
                show={true}
                closeButton={false}
