...
interface SurveillanceProfileCancelledResponse {
  data: SurveillanceProfileState[];
  totalPage: number;
  totalItem: number;
}

export const useCancelledSurveillanceProfiles = () => {
  const { showLoading, hideLoading } = useLoadingContext();
  const [surveillanceProfiles, setSurveillanceProfiles] =
    useState<SurveillanceProfileCancelledResponse>({
      data: [],
      totalPage: 0,
      totalItem: 0,
    });
  ...
  const fetchCompletedSurveillanceProfiles = useCallback(async () => {
    try {
      ...
      const response = await API.get(
        endpoint['cancelledSurveillanceProfiles'],
        { params },
      );
      ...
      setSurveillanceProfiles({
        data:
          response?.data?.map((item: SurveillanceProfileResponse) => ({
            ...
            cancel_reject_reason: item.cancel_reject_reason || '',
          })) || [],
