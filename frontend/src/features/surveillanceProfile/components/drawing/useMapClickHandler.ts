...
import { convertLineToWaypoints } from '@/features/surveillanceProfile/utils/convertLineToWaypoints';
...
export const useMapClickHandler = ({
  ...
  const setDrawingModeWithoutClear = useDrawingModeStore(
    (state) => state.setDrawingModeWithoutClear,
  );
  ...
  const handleMapRightClick = useCallback(() => {
    if (drawingMode === 'LINE' && drawingPoints.length >= 2) {
      // Complete the line drawing and create waypoints
      const lineWaypoints = convertLineToWaypoints(drawingPoints);
      setWaypointsList(lineWaypoints);
      setIsEditable(true);
      setDrawingPoints([]);
      // Use setDrawingModeWithoutClear to avoid clearing waypoints when finishing LINE mode
      setDrawingModeWithoutClear('NONE');
      // Also update the local drawing mode to stop the drawing action
      setDrawingMode('NONE');
    } else if (
      drawingMode === 'TRACE' &&
      isDrawingActive &&
