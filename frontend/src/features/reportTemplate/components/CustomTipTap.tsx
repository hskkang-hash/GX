...
// import HardBreak from '@tiptap/extension-hard-break'; // Removed to prevent automatic br generation
...
import { CodeBlockExtension } from '../utils/CodeBlockExtension';
...
const PreserveClassExtension = Extension.create({
  ...
  addGlobalAttributes() {
    return [
      {
        types: ['paragraph', 'heading', 'listItem', 'table'],
        attributes: {
          class: {
            default: null,
...
// Custom extension to preserve table classes
const TableClassExtension = Extension.create({
  name: 'tableClass',
...
export const CustomTiptap: React.FC<CustomTiptapProps> = ({
  ...
  const editor = useEditor({
    ...
    onUpdate({ editor }) {
      if (onChange) {
        // Use getHTML() instead of innerHTML to preserve proper table structure
        const editorContent = editor.getHTML();
        let editorHTML = `<div class="tiptap-preview">${editorContent}</div>`;
  ...
  useEffect(() => {
    if (editor && content !== localContent) {
      ...
      // Parse content to ensure proper table structure
  ...
  // Kiểm tra xem có đang ở trong code block không
  ...
  const toggleTableBorders = (): void => {
    ...
    while (current && current !== editorElement) {
      if (current.nodeName === 'TABLE') {
        ...
        // Update the table's class attribute in the editor state
        const tablePos = editor.view.posAtDOM(currentTableElement, 0);
        const tableNode = editor.state.doc.nodeAt(tablePos);

        if (tableNode) {
          const newClass = hasHiddenBorders ? '' : 'hide-table-borders';
          editor.commands.updateAttributes('table', { class: newClass });
        }
        ...
        <div className="tiptap-toolbar">
          <div className="toolbar-group">
            <button
              ...
              onClick={() =>
                !isInCodeBlock() && editor.chain().focus().toggleBold().run()
              }
            ...
            <button
              ...
              onClick={() =>
                !isInCodeBlock() && editor.chain().focus().toggleItalic().run()
              }
              disabled={isInCodeBlock()}
              title="Italic"
              type="button"
            ...
            <button
              ...
              onClick={() =>
                !isInCodeBlock() && editor.chain().focus().toggleStrike().run()
              }
              disabled={isInCodeBlock()}
              title="Strike"
              type="button"
          ...
          <div className="toolbar-group">
            <button
              ...
              onClick={() =>
                !isInCodeBlock() && editor.chain().focus().toggleCode().run()
              }
              disabled={isInCodeBlock()}
              title="Inline Code"
              type="button"
          ...
          {fieldsTemplate && fieldsTemplate.length > 0 && (
            ...
            >
              {fieldsTemplate.map((field: FieldTemplate, index: number) => (
                <button
