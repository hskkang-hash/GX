...
import { getGeographicCenterAndZoomGoogle } from '../../routes/utils/calculateCenterAndZoom';
...
const MapForRouteGoogleContent = ({
  ...
  const storeDrawingMode = useDrawingModeStore((state) => state.drawingMode);

  // Update store when drawing mode changes
  useEffect(() => {
    setStoreDrawingMode(drawingMode);
    // Clear drawingPoints when drawingMode changes to NONE to complete the line
    if (drawingMode === 'NONE') {
      setDrawingPoints([]);
    }
  }, [drawingMode, setStoreDrawingMode, setDrawingPoints]);

  // Sync drawingMode from store (when FormSurveyMission sets it to NONE after editing)
  useEffect(() => {
    if (storeDrawingMode !== drawingMode) {
      setDrawingMode(storeDrawingMode);
      // Clear drawingPoints when drawingMode is set to NONE from store
      if (storeDrawingMode === 'NONE') {
        setDrawingPoints([]);
      }
    }
  }, [storeDrawingMode, drawingMode, setDrawingMode, setDrawingPoints]);
  ...
  useEffect(() => {
    if (storeCurrentShape && !currentShape) {
      ...
      // Also sync drawingPoints for LINE mode
      if (storeCurrentShape.type === 'LINE' && storeCurrentShape.points) {
        setDrawingPoints(storeCurrentShape.points);
      }
    } else if (storeCurrentShape && currentShape) {
      // Check if shape was updated (different ID or different points)
      const hasIdChanged = storeCurrentShape.id !== currentShape.id;
      const hasPointsChanged =
        storeCurrentShape.type === 'LINE' &&
        storeCurrentShape.points &&
        currentShape.points &&
        (storeCurrentShape.points.length !== currentShape.points.length ||
          storeCurrentShape.points.some(
            (point, index) =>
              !currentShape.points?.[index] ||
              Math.abs(point.lat - currentShape.points[index].lat) > 0.000001 ||
              Math.abs(point.lng - currentShape.points[index].lng) > 0.000001,
          ));

      if (hasIdChanged || hasPointsChanged) {
        // Sync the shape from store
        setCurrentShape(storeCurrentShape);
        // Always sync drawingPoints for LINE mode to ensure line updates correctly
        if (storeCurrentShape.type === 'LINE' && storeCurrentShape.points) {
          setDrawingPoints([...storeCurrentShape.points]); // Create new array reference
        }
      }
    }
  }, [
  ...
  console.log('centerMap', centerMap, zoomLevel);
